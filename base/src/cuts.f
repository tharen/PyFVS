      ...
      IMPLICIT NONE
C----------
C BASE $Id: cuts.f 1399 2015-01-09 00:44:58Z tod.haren $
C----------
...
C----------
     ...
     >          DSNG(MAXTRE),CTCRWN(MAXTRE),TKCRWN(MAXTRE)
      REAL BASNEW,CUTMAX,FTCUT,BRBASE,CRLEN,CRI,HTI,DHI,DLOW,PPROP
...
C  ACTIVITIES 201,202,203,222,223,224,225,226,227,228,229,230,231,232,
C             233,234,235,236,237,248
C----------
      CALL OPFIND (21,MYACTS(2),NTODO)
      IF (DEBUG) WRITE (JOSTND,9020) NTODO,(MYACTS(I),I=2,22)
...
C  BRANCHING HERE FOR THINAUTO IN COMPUTED "GO TO" WON'T HAPPEN
C----------
  200 CONTINUE
      LBELOW=.TRUE.
  225 CONTINUE
...
C----------
  355 CONTINUE
      ...
      IF((ICFLAG.EQ.17).AND.(QFATAR.LE.1)) GO TO 360
C----------
...
C----------
      IF((REMOVE.GT.0.0).OR.(LPTALL.AND.(JPNUM.LT.IPTINV))
     &    .OR.(LPTGROUP.AND.(JPT.LT.IPTGRP(JPTGRP,1)))) GO TO 650
      CALL OPDEL1(KUT)
      GO TO 1400
  650 CONTINUE
C------------
...
C----------
 ...
 1550 CONTINUE
C----------
C  IF THE PROBABILITY OF REMOVAL IS TRIVIAL, DON'T CUT THE TREE
C----------
...
C----------
 ...
 2022 CONTINUE
C----------
C SKIP TREE IF IT DOESN'T MATCH SPECIFIED SPECIES OR SIZE
C----------
