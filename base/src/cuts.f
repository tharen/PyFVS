...
C----------
     ...
     >          DSNG(MAXTRE),CTCRWN(MAXTRE),TKCRWN(MAXTRE)
      ...
      REAL BACUT,BFCUT,CFCUT,TCUT,ZSDI,CCCLC1,CCCLC21,CCCSDI,CCT
      REAL CRADIF,CCCDIF
      ...
      INTEGER ISHAG,QFATAR,JOSTND1,ICFLAGQFA,JPTGRP,JPT,IBA
      CHARACTER VVER*7
...
C----------
      ...
      ENDIF
C----------
C  IF CCADJ IS USED WITH THINCC, CALL SSTAGE TO RESET CCCOEF 
C----------
      IBA=1
      IF(GLOCCC.EQ.1.AND.ICFLAG.EQ.11)CALL SSTAGE(IBA,ICYC,.TRUE.)
C----------
...
C
        IF(SDIC .GT. CSDI) THEN
          IF(GLOCCC.EQ.2.OR.CCCOEF.NE.0.01)THEN           
...
C                  DON'T LEAVE THE SECIES = .FALSE.
C----------
 1340 CONTINUE
