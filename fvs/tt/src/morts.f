...
C----------
      ...
      INTEGER I2,I3,IS,IPATH,IDMFLG,KNT2,ITODO,KPOINT,KBIG,IGRP,IULIM
      INTEGER IG,IX,J,IP,IPDG,IPDG2
      REAL PRM(6)
      REAL PMSC(MAXSP),PMD(MAXSP)
      REAL TMD0,T85D0,T55D0,TMD10,T85D10,T55D10,TN10,TEM,D55M,T55M
      REAL D85M,T85M,SLP,CEPT,TPRIME,DIFF,RN,XMORT,D1,D2,B0,B1,WKI,RI
      REAL RIP,X,PRES,VLOS,SUMTRE,TN,SD2SQN,DQ10N,TREEIT,DQ0,BOOST
      REAL SUMKIL,T,SD2SQ,P,D,BARK,G,CIOBDS,DQ10,DELTBA,CONST
      REAL BRATIO,TNEW,BANEW,QMDNEW,ADJFAC,XMORE,TEMP,CREDIT,TN10TMP
      REAL TMMSB,T85MSB,TMORE,TEMEFF,TPACLS,DBHEND,BADEAD,POT(54)
      REAL BA10,TB,TTB,RZ,DSUM,WPROB,AVED,RELDBH,DGT,POTENT,RIPP,XCHECK
...
C----------
      DATA PMSC/
     & 6.5112, 6.5112, 7.2985, 5.1677, 9.6943,
     & 5.1677, 5.9617, 9.6943, 5.1677, .21180,
     & 5.1677, 5.1677, 5.5877, 5.1677, 5.9617,
     & 5.9617, 5.1677, 5.9617/
      DATA PMD/
     & -.0052485, -.0052485, -.0129121, -.0077681, -.0127328,
     & -.0077681, -.0340128, -.0127328, -.0077681,        0.,
     & -.0077681, -.0077681, -0.005348, -.0077681, -.0052485,
     & -.0340128, -.0077681, -.0052485/
C----------
C  HABITAT TYPE CONSTANTS USED FOR PONDEROSA PINE FROM THE CI VARIANT.
C  PAYETTE NF DEFAULTS
C----------
      DATA IPDG/12/
C
      DATA IPDG2/41/   
...
C
      ...
      DQ0=SQRT(SDQ0/T)
      DQ10=SQRT(SD2SQ/T)
      IF(DEBUG)WRITE(JOSTND,*)'TESTMORTS,249,T,ISISP',T,ISISP
      IF(LZEIDE)THEN
...
C
 ...
 9020 FORMAT('IN MORTS  DQ0 = ',F8.2,' DQ10 = ',F10.2,
     &' T = ',F8.2,' SD2SQ = ',F10.2)
C----------
C  SPECIAL NEEDS SECTION:
C
C  THE FOLLOWING IS USED FOR PONDEROSA PINE FROM THE CI VARIANT --
...
C
      SELECT CASE (ISPC)
C
C  PONDEROSA PINE FROM THE CI VARIANT
C
      CASE(10)
...
C----------
     ...
     &      11.2007*G-0.554421/D+B0+0.246301*RELDBH+6.07129*G/D
        IF(RIP.GT.88.5) RIP = 88.5
        IF(RIP.LT.-88.5) RIP=-88.5
        RIP=(1.0/(1.0+EXP(RIP)))
        POTENT=REIN(IP)
        RIP=RIP*POTENT
        RIPP=BA*RZ
...
C----------
        ...
        PRES=P-WKI
        VLOS=WKI*CFV(I)/FINT
        WRITE(JOSTND,9002) I,ISPC,D,G,RELDBH,BA,P,WKI,PRES,VLOS
 9002   FORMAT('IN MORTS, I=',I4,',  ISPC=',I3,',  DBH=',F7.2,
...
C----------
      ...
      IF(TN10 .GE. 0.1)THEN
        CALL TTMRT(SUMTRE,DEBUG,SUMKIL)
      ENDIF
C----------
...
9001  CONTINUE
      ...
      G = (DG(I)/BARK) * (FINT/10.0)
      IF(DEBUG)WRITE(JOSTND,*)'I,D,DG,BARK,PROB,WK2,P= ',
     &I,D,DG(I),BARK,PROB(I),WK2(I),P 
      TNEW=TNEW+P
