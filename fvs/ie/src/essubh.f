     ...
     &  GENTIM,TRAGE)
      IMPLICIT NONE
C----------
C  **ESSUBH--NI23   DATE OF LAST REVISION:   01/27/09
C----------
C
COMMONS
C
...
C
C     HT OF TALLEST SUBSEQUENT WWP. 6JAN88 CARLSON
C
   10 CONTINUE
...
C
   20 CONTINUE
     ...
     &    +UPRE(IPREP,2)
      ...
      GO TO 300
C
C     HT OF TALLEST SUBSEQUENT D-FIR. 8JAN88 CARLSON
C
   30 CONTINUE
     ...
     &    -0.23946*XSIN -0.14589*SLO
      ...
      GO TO 300
C
C     HT OF TALLEST SUBSEQUENT GRAND FIR. 6JAN88 CARLSON
C
   40 CONTINUE
     ...
     &    +0.05070*ELEV -0.000736*ELEV*ELEV
      ...
      GO TO 300
C
C     HT OF TALLEST SUBSEQUENT W. HEMLOCK. 7JAN88 CARLSON
C
   50 CONTINUE
      ...
      GO TO 300
C
C     HT OF TALLEST SUBSEQUENT W.CEDAR.
C
   60 CONTINUE
     ...
     &    -0.01594*ELEV
      ...
      GO TO 300
C
C     HT OF TALLEST SUBSEQUENT LPP. 7JAN88 CARLSON
C
   70 CONTINUE
     ...
     &    +0.01105*BWAF +0.02588*BWB4
      ...
      GO TO 300
C
C     HT OF TALLEST SUBSEQUENT E. SPRUCE. 7JAN88 CARLSON
C
   80 CONTINUE
     ...
     &    +UPHY(IPHY,8) +UHAB(IHTSER,8)
      ...
      GO TO 300
C
C     HT OF TALLEST SUBSEQUENT SAF.  6JAN88 CARLSON
C
   90 CONTINUE
...
C
  100 CONTINUE
     ...
     &    +UPRE(IPREP,10) -0.01155*ELEV
      ...
      GO TO 300
C
C     HT OF TALLEST SUBSEQUENT M. HEMLOCK (USES WH EQUATION).
C
  110 CONTINUE
      PN= -2.42379 +1.52366*AGELN -0.003256*BAA
      HHT = EXP(PN +EMSQR*DILATE*BNORM*0.54116)
      GO TO 300
C
C     HT OF TALLEST SUBSEQUENT WHITEBARK PINE (USES WL)
C
  120 CONTINUE
      PN= -1.36257 +1.21548*AGELN -0.003797*BAA +UHAB(IHTSER,2)
     &    +UPRE(IPREP,2)
      ...
      GO TO 300
C
C     HT OF TALLEST SUBSEQUENT LIMBER PINE
C
  130 CONTINUE
      PN = 0.0
      HHT = 0.5
      GO TO 300
C
C     HT OF TALLEST SUBSEQUENT SUBALPINE LARCH (USES SAF)
C
  140 CONTINUE
      PN= -2.06377 +1.18184*AGELN -0.0044465*BAA +0.06615*XCOS
     &    +0.03085*XSIN -0.37402*SLO
      ...
      GO TO 300
C
C     HT OF TALLEST SUBSEQUENT PINYON
C
  150 CONTINUE
      PN = 0.0
      HHT = 0.5
      GO TO 300
C
C     HT OF TALLEST SUBSEQUENT JUNIPER
C
  160 CONTINUE
      PN = 0.0
      HHT = 0.5
      GO TO 300
C
C     HT OF TALLEST SUBSEQUENT PACIFIC YEW
C
  170 CONTINUE
      PN = 0.0
      HHT = 0.5
      GO TO 300
C
C     HT OF TALLEST SUBSEQUENT ASPEN
C
  180 CONTINUE
      PN = 0.0
      HHT = 5.0
      GO TO 300
C
C     HT OF TALLEST SUBSEQUENT COTTONWOOD
C
  190 CONTINUE
      PN = 0.0
      HHT = 5.0
      GO TO 300
C
C     HT OF TALLEST SUBSEQUENT MOUNTAIN MAPLE
C
  200 CONTINUE
      PN = 0.0
      HHT = 5.0
      GO TO 300
C
C     HT OF TALLEST SUBSEQUENT PAPER BIRCH
C
  210 CONTINUE
      PN = 0.0
      HHT = 5.0
      GO TO 300
C
C     HT OF TALLEST SUBSEQUENT OTHER HARDWOOD
C
  220 CONTINUE
      PN = 0.0
      HHT = 5.0
      GO TO 300
C
C     HT OF TALLEST SUBSEQUENT OTHER SOFTWOOD (USES MH)
C
  230 CONTINUE
      PN= -2.42379 +1.52366*AGELN -0.003256*BAA
      HHT = EXP(PN +EMSQR*DILATE*BNORM*0.54116)
