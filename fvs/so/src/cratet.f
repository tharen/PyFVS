...
C  FOR SPECIES USING LOGIC FROM THE WC VARIANT -
...
C
      ...
      ENDIF
C
C  LOGIC FOR SPECIES FROM WC VARIANT (SEPARATE EQS. FOR SMALL TREES)
C
      SELECT CASE(ISPC)
      CASE (15,19:23,25,26,28:31,33)
        IF(D .LT. 5.0) THEN
...
C
      ...
      END SELECT
C
C  SMALL PP USES A DIFFERENT LOGIC. RJ 7-26-88
C
      IF(ISPC.EQ.10 .AND. D.LT.3.0) THEN
      IF(JCR .GT. 7)JCR=7
      H = 8.31485 + 3.03659*D - 0.59200*JCR
