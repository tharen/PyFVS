      ...
      IMPLICIT NONE
C----------
C   **FMCFMD FIRE-CA-DATE OF LAST REVISION:  04/27/11
C----------
...
*     FUEL MODEL
*     THIS VARIANT IS VERY SIMILAR TO THE NC-FFE VERSION
*----------------------------------------------------------------------
*
...
C     LOCAL VARIABLE DECLARATIONS
      ...
      PARAMETER(ICLSS=13)  ! NUMBER OF FUEL MODELS
      PARAMETER(CWHRR=11)  ! ROWS IN CWHR MATRIX
      PARAMETER(CWHRC=18)  ! COLS IN CWHR MATRIXC

      CHARACTER SZDN*3, FTLABL(0:CWHRR)*2, SSLABL(0:CWHRC)*2, SDTMP*3
...
C     Row labels: "P","R","WE","WW","D","J","H","X","MP","MC","OS"
     ...
     >  5, 5, 5, 6, 6, 6, 6, 6, 8, 6, 6, 8, 8, 6, 6, 8, 8,10 /!11 other soft
...
C     FIND THE FOREST TYPE ROWS (IFT) FOR THE FIRE BEHAVIOUR MODEL TABLE
      ...
      IFT = 0
      IF (BAPCT(18) .GE. 80.0) THEN
        IFT = 1                               ! pine
      ELSE
        IF ((BAPCT(4)+BAPCT(5)+BAPCT(6)) .GE. 80.0) THEN
          IF ((BAPCT(5)+BAPCT(6)) .GE. BAPCT(4)) THEN
             IFT = 2                          ! red fir
          ELSE
          ...
          ENDIF
        ELSE
          IF (BAPCT(7)      .GE. 80.0) THEN   ! Douglas-fir
            IFT = 5
          ELSEIF (BAPCT(15) .GE. 80.0) THEN   ! Jeffrey pine
            IFT = 6
          ELSEIF (XHARD     .GE. 80.0) THEN   ! hardwoods
            IFT = 7
          ELSEIF (BAPCT(12) .GE. 80.0) THEN   ! lodgepole pine
            IFT = 8
          ELSEIF (XMCON     .GE. 80.0) THEN   ! mixed conifer
            IFT = 10
          ELSE
            IF (BAPCT(18) .GT. BAPCT(4)) THEN ! mixed pine
              IFT = 9
            ELSE
              IFT = 11                        ! other softwood
            ENDIF
          ENDIF
        ENDIF
      ENDIF
