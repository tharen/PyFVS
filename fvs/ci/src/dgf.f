...
C----------
     ...
     &   DGSLSQ(MAXSP),DGBAL(MAXSP),DGCCFA(MAXSP)
      ...
      REAL D,CR,BAL,PBAL,DDS,PPBAL,PPBALD,XPPDDS,DHAB,DGCRS2,BRATIO
...
C----------
      ...
      ENDIF
      XPPDDS=0.
      CALL PPDGF (XPPDDS,PPBAL,RELDEN,BA,D,DGPCFS,DGLBAS,PPBALD)
C
      DDS=DDS+XPPDDS
C---------
      IF(DDS.LT.-9.21) DDS=-9.21
...
C----------
      ...
      WRITE(JOSTND,9001) I,ISPC,D,BAL,CR,XPPDDS,PCCF(IPCCF),BA,
     &                   PBAL,DDS
 9001 FORMAT(' IN DGF, I=',I4,',  ISPC=',I3,',  DBH=',F7.2,
     &      ',  BAL=',F7.2,',  CR=',F7.4,', XPPDDS=',F5.3/
