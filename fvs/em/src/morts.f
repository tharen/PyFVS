      ...
      IMPLICIT NONE
C----------
C  **MORTS--EM   DATE OF LAST REVISION:  09/09/13
C----------
...
C----------
      ...
      REAL BRATIO,BANEW,QMDNEW,ADJFAC,XMORE,TEMP,CREDIT,TN10TMP
...
C  17=PB, 18=OS, 19=OH
C
C  AS WAS DONE IN THE IE VARIANT:
C  LM,GA,AS,CW,BA,PW,NC,PB,OH USES 60 PERCENT OF THE NORTH IDAHO MH RATE
C  LL USES THE NORTH IDAHO AF RATE
C  RM USES 20 PERCENT OF THE NORTH IDAHO MH RATE
C
C  FOR THE VARIABLES FROM THE IE/NI VARIANT REFERENCED BY FOREST CODE,
C  USE: 
C  THE BITTERROOT MAPPING FOR THE BEAVERHEAD, CUSTER, DEERLODGE,
C                 AND GALLATIN
C  THE LOLO       MAPPING FOR THE HELENA AND LEWIS AND CLARK
C----------
C  DATA STATEMENTS.
C
      DATA MYACTS/94,97/
...
C
     ...
     &          0.0,        0.0,        0.0,        0.0/
C
C IN THE ARRAYS IPDG AND IPDG2, THE I INDEX REFERS TO THE NORTH IDAHO,
C HABITAT TYPE AND THE INDEX J REFERS TO NATIONAL FOREST.
C
      DATA ((IPDG(I,J),I=1,30),J=1,6)/
C BITTERROOT MAPPING USED FOR THE BEAVERHEAD
     &  7, 5*6, 5,6,5, 4*6, 8, 5*7, 5,3,4,3,4,4,6,5,1,1,6,
C BITTERROOT MAPPING USED FOR THE CUSTER
     &  7, 5*6, 5,6,5, 4*6, 8, 5*7, 5,3,4,3,4,4,6,5,1,1,6,
C BITTERROOT MAPPING USED FOR THE DEERLODGE
     &  7, 5*6, 5,6,5, 4*6, 8, 5*7, 5,3,4,3,4,4,6,5,1,1,6,
C BITTERROOT MAPPING USED FOR THE GALLATIN
     &  7, 5*6, 5,6,5, 4*6, 8, 5*7, 5,3,4,3,4,4,6,5,1,1,6,
C LOLO       MAPPING USED FOR THE HELENA
     &  4*7, 6,7,5,6,7,6,6,7,7,9,9,8,9,8,8,5,3,5,4,5,5,7,5,2,4,7,
C LOLO       MAPPING USED FOR THE LEWIS AND CLARK
     &  4*7, 6,7,5,6,7,6,6,7,7,9,9,8,9,8,8,5,3,5,4,5,5,7,5,2,4,7/
C
C
C OTHER IPDG MAPPINGS THAT R1 MAY WANT TO USE:
C
C IDAHO PANHANDLE
C    &  4*15, 14,14,12,14,13,14,14,15,15,17,15,15,16,15,15,11,10,11,9,
C    &     12,12,15,12,7,10,15,
C CLEARWATER
C    &  4*14, 13,13,11,14,12,14, 3*14, 17,14,14, 3*15, 11,9,10,8,11,11,
C    &     15,11,5,10,15,
C COEUR DE'ALENE
C    &  4*12, 11,11, 3*10, 11,11,12,12, 3*14, 3*13, 9,6,8,7,9,10,12,10,
C    &     4,8,12,
C COLVILLE
C    &  4*10, 9,9,8,9,8,9, 3*10, 12, 5*11, 8,6,7,5,7,7,10,8,6,7,10,
C FLATHEAD
C    &  6*9, 7, 3*8, 3*9, 11, 4*10, 11,7,5,6,4,7,7,9,8,3,6,9,
C KANIKSU
C    &  13, 5*12, 10,12, 3*11, 12,12,15,13,13,14,13,13,9,8,9,7,10,10,
C    &     12,10,4,8,12,
C KOOTENAI
C    &  4*9, 8,9,7,8,7, 3*8, 9,11, 5*10, 7,5,6,5,7,7,9,8,3,6,9,
C NEZ PERCE
C    &  4*15, 14,14,9,13,13,14,14,13,14,18, 5*14, 10,9,9,8,11,11,15,9,
C    &     6,8,15,
C ST JOE
C    &  4*15, 14,14,12,14,13,14,14,15,15,17,15,15,16,15,15,11,10,11,9,
C    &     12,12,15,12,7,10,15
...
C
C
C OTHER IPDG2 MAPPINGS THAT R1 MAY WANT TO USE:
C
C IDAHO PANHANDLE
C    &  50, 3*49, 48,48,47,52,46,47,48,53,54,54,52,52,54,50,51,41,39,44,
C    &     43,44,44,50,53,36,37,50/
C CLEARWATER
C    &  4*45, 44,44,43,48,43,43,44,49,50,50, 3*49, 47,47,39,36,40,39,40,
C    &     39,45,41,33,34,45,
C COEUR DE'ALENE
C    &  4*41, 40,40,39,42,39,39,40,44, 5*45, 43,44,36,33,36,36,37,38,41,
C    &     38,30,31,41,
C COLVILLE
C    &  38, 3*37, 36,37,37,40,35,36,37,41,41,42,40,40,41,40,40,33,30,33,
C    &     31,32,31,36,34,32,27,36,
C FLATHEAD
C    &  38, 3*37, 3*36, 38,36,35,37, 3*41, 40,40,41,40,40,33, 3*32, 34,
C    &     33,38,35,26,28,38,
C KANIKSU
C    &  4*41, 40,40,39,45,37,38,39,43,44,46,43,43,45,43,43,35,33,35,35,
C    &     36,36,41,37,30,32,41,
C KOOTENAI
C    &  38, 6*37, 41,33,37,37,41,41,42,39,39,41,40,40,33,32,33,33,34,33,
C    &     38,35,28,32,38,
C NEZ PERCE
C    &  4*45, 44,44,43,48,41,42,43,47,48,50,47,47,46,44,44,36,34,37,37,
C    &     38,37,43,34,30,33,43,
C ST JOE
C    &  50, 3*49, 48,48,47,52,46,47,48,53,54,54,52,52,54,50,51,41,39,44,
C    &     43,44,44,50,53,36,37,50,
C
...
C----------
C ORIGINAL EM SPECIES USE DIFFERENT LOGIC THAN THE ADDED SPECIES
C----------
...
C----------
   ...
   29   CONTINUE
C----------
C  APPLY 60 PERCENT OF NI RATE FOR LM,GA,AS,CW,BA,PW,NC,PB,OH
C  APPLY 20 PERCENT OF NI RATE FOR RM
C  USE FULL NI RATE FOR LL
C----------  
        IF(ISPC.EQ.5)THEN
...
C  TIME RESOLUTION.  ITYPE IS A NI HABITAT INDEX THAT IS COMPUTED IN
C  **HABTYP** .
C----------
      IPT=IPDG(ITYPE,IFOR)
      POTEN=POT(IPT)
