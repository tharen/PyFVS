...
C     must be changed from 11 to 38 for Central Rockies variant.
...
C
C  Revision History :
C  01-MAY-97 - Matthew K. Thompson (FHTET)
C     Fixed an error with setting the disease type in the PLOTINF keyword.
C  28-MAY-97 - Matthew K. Thompson (FHTET)
C     Removed keyword DATELIST and call to subroutine RDTLS.
C  21-OCT-97
C     Added the code to set IRTSPC for the Central Rockies variant.
C  02-JUN-98 - Robert Nathan Havis (FHTET)
C     Put in warning message print to output file when RRTYPE is not used
C     to specify disease and default disease type, annosus is used.
C     New logical (LRTYPE)
C  18-JAN-00 - Lance David (FHTET)
C     Inserted a keyword spaceholder in position 27 of array TABLE and
C     increased the array size to 42. Removal of the keyword that occupied
C     this space and reducing the array size (possibly mod dated 05/38/97)
C     caused the FVS option processor to report the wrong keyword in the
C     Activity Schedule and Summary reports for all Root Disease activity
...
C        Increased Format 606 for 38 species (LRD)
  606    FORMAT (38F5.2)
C
      ELSE
...
C        Increased Format 711 for 38 species (LRD)
  711    FORMAT (38F5.2)
...
C     to the output file (RNH June98)
C
...
C....    ALL SPECIES GET CHANGED
 ...
 1355       CONTINUE
 1357    CONTINUE
C        Increased Format 1321 for 38 species (LRD)
 1321    FORMAT (38F5.2)
      ELSE
...
C        Increased Format 2921 for 38 species (LRD)
 2921    FORMAT (38F5.2)
...
C....       Change all species
 ...
 3963       FORMAT(38I5)
C           Increased Format 3963 for 38 species (LRD)
