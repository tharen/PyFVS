...
C----------
C  **RDIN--IE        LAST REVISION:  06/03/10
C----------
C
C  Purpose :
...
C
C     Format statements changed for the Inland Empire 23 species variant
C     by Lance David 08/08/2003.
C
C  Called By :
C     INITRE  [PROGNOSIS]
C
...
C.... OUTPUT FOR PSTUMP KEYWORD
  ...
  301 CONTINUE
      IF (LMTRIC) THEN
         IF(LKECHO)WRITE(JOSTND,304) KEYWRD, IDT, PRMS(1),
     &                               PRMS(2) * INTOCM
...
C.... OUTPUT FOR RRINIT KEYWORD
  ...
  572       FORMAT (T13,'NUMBER OF CENTERS=',I4/
     ...
     &            T13,'UNINFECTED TREES/HA IN DISEASED AREAS = ',F7.2)
         ELSE
            IF(LKECHO)WRITE(JOSTND,542) NCENTS(IDI), PRKILL(IDI),
     &                                  PRUN(IDI)
...
C.... array to the user defined value and then print out the value.
      ...
      DO 1820 IDI = 1, ITOTRR
         IF (LNOTBK(IDI)) THEN
            ...
            IF(LKECHO)WRITE(JOSTND,1830) KEYWRD, DISTYP(IDI),
     &                                   XMINLF(IDI)
         ENDIF
 1820 CONTINUE
...
C....       OUTPUT FOR RRMINK KEYWORD
            ...
            IF(LKECHO)WRITE(JOSTND,2401) KEYWRD, XMINKL(IDI),
     &                                   DISTYP(IDI)
         ENDIF
 2420 CONTINUE
...
C.... OUTPUT FOR SMCOUT KEYWORD
      ...
      IF (LMTRIC) THEN
         IF(LKECHO)WRITE(JOSTND,3701) KEYWRD, SDNORM/ACTOHA,
     &                                SDISLP*ACTOHA
      ELSE
