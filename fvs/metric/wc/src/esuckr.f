...
C
      ...
      REAL TPATOT,PREM,SPA,BACHLO,BX,AX,CRDUM,CW,RSHAG
      EXTERNAL ESRANN
      LOGICAL DEBUG
      CHARACTER CLABEL*2
...
c		(in debug statement, changed to aspent sprouts per ha)
c		(ASBAR = ASPEN BASAL AREA REMOVED)
C
     ...
     &        - 3.5208*RSHAG**5.0 + 0.011797*RSHAG**7.0
        IF(DEBUG)WRITE(JOSTND,*)' AFTER CALC SPA= ',SPA/ACRtoHA
     ...
     &  I,ISHAG,SPA/ACRtoHA,ASBAR*FT2pACRtoM2pHA,ASTPAR,PREM
      ENDIF
C
...
C----------
C  PACIFIC YEW (33) SPROUT COUNT IS CUT IN HALF
C----------
      IF(ISSP.EQ.33)THEN
        NUMSPR=1
      ELSE
