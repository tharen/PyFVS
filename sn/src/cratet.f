      ...
      implicit none
C----------
C  **CRATET--SN  DATE OF LAST REVISION:  04/06/11
C----------
...
C----------
      ...
      INTEGER IXF,I,J,JJ,K,II,ISPC,IPTR,I1,I2,I3,K1,K2,K3,K4,NH,IS,IM
      REAL SPCNT(MAXSP,3)
      REAL SDIFIA,AX,Q,SUMX,H,D,BX,XX,YY,XN,HS
...
C----------
C  FIND INTITIAL FOREST TYPE FOR CYCLE 0 AND LOAD SDIDEF ARRAY
C  AND SET BAMAX.
C----------
      IXF=3
      CALL FORTYP(IXF,SDIFIA)
      IF(DEBUG)WRITE(JOSTND,*)'IXF, SDIFIA= ',IXF, SDIFIA
      DO 11 I=1,MAXSP
      IF(SDIDEF(I) .GT. 0.) THEN
        MAXSDI(I)= 1
      ELSEIF (BAMAX .GT. 0.)THEN
        LBAMAX= .TRUE.
...
C----------
   ...
   93 CONTINUE
C-------
C  IF THERE ARE TREE RECORDS, BRANCH TO PERFORM CALIBRATION.
C-------
      AX=0.
      IF (ITRN.GT.0) GOTO 1
C----------
