      ...
      IMPLICIT NONE
C----------
C  $Id: dmcwcr.f 594 2013-03-08 21:36:18Z drobinsonessa@gmail.com $
C----------
...
C   selected, and adjust accordingly.
C
...
c this is totally faked: PP is in position 13, and
c **only** that position is guaranteed.
...
C.... Mistletoe model initializations for CR subvariant

c      IF (ITYPE .LT. 1 .OR. ITYPE .GT. 5) THEN
c         LTYPE = 1
c      ELSE
c         LTYPE = ITYPE
c      ENDIF
c
c      IF (LTYPE .EQ. 1) THEN
c         BH1 (7) = BH1 (8)
c         BINT2(7)= BINT2(8)
c         BH2 (7) = BH2 (8)
c      ENDIF
c
c     IF (LTYPE .EQ. 2) THEN
c         BH1 (7) = BH1 (8)
c         BINT2(7)= BINT2(8)
c         BH2 (7) = BH2 (8)
c         DO 200 I=8,9
