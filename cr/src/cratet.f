...
C----------
      ...
      REAL SPCNT(MAXSP,3),YOUNG,OLD
      REAL AX,Q,SUMX,H,D,BX,XX,YY,XN,SI,CONST,HS
...
C
C SPECIAL BLACK HILLS MODIFICATION TO HEIGHT DUBBING LOGIC.
C BH DEFAULT EQUATION FOR ALL SPECIES, USE CALIBRATED LOGISTIC EQUATION
C IF ENOUGH OBSERVATIONS EXISTED TO CALIBRATE IT
C
      IF(IMODTY.EQ.3 .AND. IABFLG(ISPC).EQ.1) THEN
        IF(D.GT.0.5)THEN
          H=32.108633*(SITEAR(ISPC)**0.276926)*
     &      ((1.0-EXP(-0.057766*D))**(0.9844340**(-0.169876)))+4.5
        ELSE
          SI = SITEAR(ISPC)
          CONST = 12.41173 + 0.04633*SI - 0.000158*SI*SI
          H = CONST * D
        ENDIF
      ENDIF
      IF(H .LE. 4.5)H=4.5
