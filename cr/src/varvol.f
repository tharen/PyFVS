...
C----------
C  SET R3VOL PARAMETERS & CALL PROFILE TO COMPUTE R3 VOLUMES.
C----------
        IF(VEQNNC(ISPC).EQ.'          ') THEN
          VN=0.
          VMAX=0.
          ...
          GO TO 102
C
        ELSEIF(VEQNNC(ISPC)(4:6).EQ.'DVE')THEN      ! R3 DVE EQ LOGIC
          IERR=0
          UNT=3
...
C
          CALL R3D2HV(VEQNNC(ISPC),UNT,H,HT1PRD,D,DRC,IFC,
     &                IHTLL,TVOL,IERR)
C
          IF(DEBUG)WRITE(JOSTND,*)' AFTER R3D2HV TVOL= ',TVOL
C
        ELSE                                        ! R3 PROFILE LOGIC
C
          DO IZERO=1,15
          TVOL(IZERO)=0.
          ENDDO
...
C
            ...
            ENDIF
C
            IF(DEBUG)WRITE(JOSTND,*)' AFTER PROFILE R3 BF TVOL= ',TVOL
            TVOL(1)=TVOL1
            TVOL(4)=TVOL4
