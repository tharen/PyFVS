...
C
      ...
      REAL    TC,SDHI,SDLO,ARRAY(7),PRMS(10),XBAF
      INTEGER ITOPD8,ITOPD9,IACT,IRC,IDBCYC,NP,K,I1,IGSP,IG,IULIM,IGRP
      ...
      CHARACTER*7 VVER, VPLEQ*10,CALCSDI
      CHARACTER*20 KARD12
      CHARACTER*4 IEND,CFOUR
...
C
  ...
  131   FORMAT (/' STDINFO    FOREST-LOCATION CODE=',I8,
     ...
     >     '; AGE=',I5,'; ASPECT AZIMUTH IN DEGREES= ',F4.0,
     >       '; SLOPE= ' ,F4.0,'%'/
     > T13,'ELEVATION (M)=',F7.1,'; LATITUDE IN DEGREES=',F5.0)
      ELSEIF (VVER(:2) .EQ. 'SN') THEN
        WRITE (JOSTND,135) KEYWRD,KODFOR,PCOM,
     >                    IAGE,ASPECT,SLOPE,ELEV*100.*FTtoM,TLAT
...
C
      ELSEIF (VVER(:2) .EQ. 'BC') THEN
        WRITE (JOSTND,132) BEC%PrettyName,IAGE,ASPECT,SLOPE,
     &                     ELEV*100.*FTtoM, ADJUSTL(CPVREF)
  132   FORMAT (/' STDINFO    BEC=',A15,
     >     '; AGE=',I5,'; ASPECT AZIMUTH IN DEGREES= ',F4.0,
     >     '; SLOPE= ' ,F4.0,'%'/
     > T13,'ELEVATION (M)=',F7.1,'; REFERENCE CODE= ',A4)
      ELSE
...
C----------
 ...
 213       FORMAT(T13,'SITE SPECIES NOT SPECIFIED.  CODE=',I5)
     ...
     &     .OR. VVER(:2).EQ.'KT' .OR. VVER(:2).EQ.'BC') THEN
           WRITE(JOSTND,214)
...
C
 ...
 1411   FORMAT (T13,A,A,' (CODE= ',I3,') ',A,
     >           ' TARGETED FOR SCREENING.')
      ELSE
         IF(LKECHO .AND. LNOTBK(5))WRITE(JOSTND,1411) 'ALL',
     >     ' SPECIES',IS,'ARE'
      ENDIF
 ...
 2400 CONTINUE
      ...
      ELSE
        IF(LNOTBK(2))THEN
          KODTYP=IFIX(ARRAY(2))
          ICL5=KODTYP
          ! concat fld1+2 to accommodate long BEC strings
          KARD12 = KARD(1)//KARD(2)
          CALL HABTYP(KARD12,ARRAY(2))
     ...
     &   VVER(:2).EQ.'LP' .OR. VVER(:2).EQ.'UT' .OR.
     &   VVER(:2).EQ.'TT' .OR. VVER(:2).EQ.'WS') THEN
         IF(LKECHO)WRITE (JOSTND,2411) KEYWRD,KODFOR,KODTYP,
...
C
      ELSEIF(VVER(:2).EQ.'BC') THEN
         IF(LKECHO)WRITE(JOSTND,24101) KEYWRD,BEC%PrettyName,IAGE,
     >     ASPECT,SLOPE,ELEV*100.*FTtoM,ADJUSTL(CPVREF)
24101    FORMAT (/1X,A8,'   BEC=',A15,
     >  '; AGE=',I5,'; ASPECT AZIMUTH IN DEGREES= ',F4.0,
     >   '; SLOPE= ',F4.0,'%'/
     > T13,'ELEVATION (M)=',F7.1,'; REFERENCE CODE= ',A4)
C
...
C
 ...
 4120   FORMAT(1X,T13,'DEFECT CORRECTIONS WILL BE APPLIED TO ',
     &  'MERCHANTABLE CUBIC METER VOLUME.')
      ENDIF
      GO TO 10
C
...
C----------
 ...
 4320   FORMAT(/1X,A8,T13,'CUBIC METER VOLUME DEFECT',
     >       ' PROPORTIONS HAVE BEEN CHANGED FOR ALL SPECIES:',
     >       /T13,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2,
     >       '; 38.1 CM TREES=',F6.2,/T13,'50.8 CM TREES=',F6.2,
     >       '; 63.5 CM AND LARGER TREES=',F6.2)
...
C----------
     ...
     >      (ARRAY(I),I=3,7)
 4332     FORMAT(/1X,A8,T13,'PULPWOOD VOLUME DEFECT PROPORTIONS HAVE ',
     ...
     >    (ARRAY(I),I=3,7)
 4340   FORMAT(/1X,A8,T13,'CUBIC METER VOLUME DEFECT PROPORTIONS HAVE ',
     >       'BEEN CHANGED FOR SPECIES:',A,' (CODE=',I3,')',
     >       /T13,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2,
     >       '; 38.1 CM TREES=',F6.2,/T13,'50.8 CM TREES=',F6.2,
     >       '; 63.5 CM AND LARGER TREES=',F6.2)
...
C----------
     ...
     &    VVER(:2).EQ.'ON') THEN
          ...
          IF(LKECHO)WRITE(JOSTND,4432) KEYWRD,KARD(2)(1:ILEN),IS,
     >      (ARRAY(I),I=3,7)
 4432     FORMAT(/1X,A8,T13,'SAWLOG VOLUME DEFECT PROPORTIONS HAVE ',
     &      'BEEN CHANGED FOR SPECIES:',A,' (CODE=',I3,')',
     &       /T13,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2,
     &       '; 38.1 CM TREES=',F6.2,/T13,'50.8 CM TREES=',F6.2,
     &       '; 63.5 CM AND LARGER TREES=',F6.2)
...
C----------
     ...
     &      DBHMIN(IGSP)*INtoCM,TOPD(IGSP)*INtoCM,STMP(IGSP)*FTtoCM,
     &      FRMCLS(IGSP)
        ENDIF
...
C----------
      ELSEIF(IS .EQ. 0)THEN
        ...
        IF (LNOTBK(4).AND.(ITOPD8.GT.0.)) ARRAY(4)=TOPD(1)
...
C
 ...
 5010   FORMAT (/1X,A8,'   DATE/CYCLE=',I5,'; CUTTING MUST EXCEED ',
     >    'THE FOLLOWING:',/,30X,F8.1,' SAWLOG M3/HA,',F8.1,
     >    ' SAWLOG BDFT,',F6.1,' M2/HA BASAL AREA, AND',
...
C
 ...
 6130   FORMAT (T13,'ONLY TREES GREATER THAN ',F7.2,
     >    ' AND LESS THAN ',F7.2,' CM DBH ARE AFFECTED.')
      ENDIF
      GOTO 10
C
...
C
 ...
 9036        FORMAT(T13,'FIXMORT MORTALITY CONCENTRATION CODE = ',F4.0,
     >       '; (BY POINT BY SIZE: LARGEST TO SMALLEST DBH)')
           ELSE
             IF(LKECHO)WRITE(JOSTND,9037)ARRAY(7)
...
C----------
 ...
 9908   FORMAT(/1X,A8,T13,'DEFAULT COEFFICIENTS FOR CUBIC METER VOLUME'/
...
C----------
 ...
 9918   FORMAT(/1X,A8,T13,'DEFAULT COEFFICIENTS FOR CUBIC METER VOLUME'/
...
C----------
 ...
 9958   FORMAT(/1X,A8,T13,'DEFAULT METRIC COEFFICIENTS FOR BOARD ',
     &    'VOLUME'/
     &    T13,'EQUATIONS REPLACED FOR ALL SPECIES.')
C----------
C     SPECIES CODE IS SPECIFIED (ISPC>0).
C----------
 ...
 9965   CONTINUE
        ILEN=3
        IF(LKECHO)WRITE(JOSTND,9968) KEYWRD,KARD(1)(1:ILEN),ISPC
 9968   FORMAT(/1X,A8,T13,'DEFAULT METRIC COEFFICIENTS FOR BOARD ',
     &   'VOLUME'/
     &    T13,'EQUATIONS REPLACED FOR SPECIES= ',A,' (CODE= ',I3,').' )
      ENDIF
C
...
C
 ...
 9977 FORMAT (/,1X,A8,'   ANNOSUS ROOT DISEASE MODEL IS OBSOLETE, ',/,
     &                ' USE WESTERN ROOT DISEASE MODEL VER. 3.0:')
      GOTO 10
C
...
11912       FORMAT(/1X,A8,T13,'PULPWOOD VOLUME DEFECT',
     >       ' PROPORTIONS HAVE BEEN CHANGED FOR ALL SPECIES:',
     >       /T13,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2,
     >       '; 38.1 CM TREES=',F6.2,/T13,'50.8 CM TREES=',F6.2,
...
11913       FORMAT(/1X,A8,T13,'SAWLOG VOLUME DEFECT',
     &      ' PROPORTIONS HAVE BEEN CHANGED FOR ALL SPECIES:',
     >       /T13,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2,
     >       '; 38.1 CM TREES=',F6.2,/T13,'50.8 CM TREES=',F6.2,
     >       '; 63.5 CM TREES=',F6.2,'; 76.2 CM TREES=',F6.2,/T13,
...
C----------
     ...
     &   VVER(:2).EQ.'ON') THEN
          IF(IDTYPE.EQ.0 .OR. IDTYPE.EQ.1)THEN
            IF(LKECHO)WRITE(JOSTND,11932) KEYWRD,KARD(2)(1:ILEN),
     >                                    IS,(WK6(I),I=1,8)
11932       FORMAT(/1X,A8,T13,'PULPWOOD VOLUME DEFECT PROPORTIONS ',
     >       'HAVE BEEN CHANGED FOR SPECIES:',A,' (CODE=',I3,')',
     >       /T13,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2,
     >       '; 38.1 CM TREES=',F6.2,/T13,'50.8 CM TREES=',F6.2,
     >       '; 63.5 CM TREES=',F6.2,'; 76.2 CM TREES=',F6.2,/T13,
     >       '88.9 CM TREES=',F6.2,'; 101.6 CM AND LARGER TREES=',F6.2)
          ...
          IF(IDTYPE.EQ.0 .OR. IDTYPE.EQ.2)THEN
            IF(LKECHO)WRITE(JOSTND,11933) KEYWRD,KARD(2)(1:ILEN),
     >                                    IS,(WK6(I),I=1,8)
11933       FORMAT(/1X,A8,T13,'SAWLOG VOLUME DEFECT PROPORTIONS HAVE ',
     &      'BEEN CHANGED FOR SPECIES:',A,' (CODE=',I3,')',
     >       /T13,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2,
     >       '; 38.1 CM TREES=',F6.2,/T13,'50.8 CM TREES=',F6.2,
     >       '; 63.5 CM TREES=',F6.2,'; 76.2 CM TREES=',F6.2,/T13,
...
C
      IF((VVER(:2).EQ.'SN').AND.(KODFOR.GE.1000))THEN
        IREGN = KODFOR/10000
        IRDUM=IREGN
...
14000 CONTINUE
      SELECT CASE(VVER(:2))
      CASE('AK','AN','BM','CA','BP','LP','SF','SM','SP',
     &      'EC','EM','NC','SN','SO','TT','UT','WS',
     &      'ON','BC')
        IF(LNOTBK(1))LZEIDE=.TRUE.
        IF(LNOTBK(2))DBHSDI=ARRAY(2)*CMtoIN
        CALCSDI="STAGE'S"
        IF(LZEIDE)CALCSDI="ZEIDE'S"
        IF(LKECHO)WRITE(JOSTND,14010) KEYWRD,CALCSDI,DBHSDI*INtoCM
14010   FORMAT (/1X,A8,'   ',A7,' SUMMATION METHOD WILL BE ',
     &              'USED TO CALCULATE SDI-BASED MORTALITY.'/
     &  9X,'   THE MINIMUM DIAMETER USED IN CALCULATIONS = ',
     &  F6.2,' CM')
        GO TO 10
      END SELECT
      SELECT CASE(VVER(:6))
        CASE('CS_NEW','LS_NEW','NE_NEW')
        IF(LNOTBK(1))LZEIDE=.TRUE.
        IF(LNOTBK(2))DBHSDI=ARRAY(2)*CMtoIN
        CALCSDI="STAGE'S"
        IF(LZEIDE)CALCSDI="ZEIDE'S"
        IF(LKECHO)WRITE(JOSTND,14020) KEYWRD,CALCSDI,DBHSDI*INtoCM
14020   FORMAT (/1X,A8,'   ',A7,' SUMMATION METHOD WILL BE ',
     &              'USED TO CALCULATE SDI-BASED MORTALITY.'/
     &  9X,'   THE MINIMUM DIAMETER USED IN CALCULATIONS = ',
     &  F6.2,' CM')
        GO TO 10
