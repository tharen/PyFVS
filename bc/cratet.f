      ...
      IMPLICIT NONE
C----------
C  $Id: cratet.f 767 2013-04-10 22:29:22Z rhavis@msn.com $
C----------
...
C
      IF (LMHTDUB(ISPC)) THEN
        H=(EXP(AA(ISPC)+BB(ISPC)/(DM+1.0))+1.3) * MtoFT
	ELSE
        H=EXP(AA(ISPC)+BB(ISPC)/(D+1.0))+4.5
      ENDIF
...
C----------
  140 CONTINUE
      ...
      WRITE(JOSTND,9005) ISPC,AA(ISPC),BB(ISPC),HT1(ISPC),HT2(ISPC)
