      ...
      IMPLICIT NONE
C----------
C  $Id: dgdriv.f 767 2013-04-10 22:29:22Z rhavis@msn.com $
C----------
...
C----------
      CHARACTER*1 CTEM(MAXSP)
      EXTERNAL RANN
      LOGICAL DEBUG,LTEM(MAXSP)
      INTEGER I,ISPC,I1,I2,I3,SFINT,ITRIPU,ITRIPL,ISI,IREFI,N
      INTEGER KOUT,K,ISPEC,KK
      INTEGER NUMCAL(MAXSP)
      REAL MISDGF,PVMLT,CORR,VARYP1,VARYP2,EVARP1,EVARP2,SIG1,RHO,RHOCP
      REAL SSIGMA,XDGROW,FRL,FRM,FRU,FRMT,CORMLT,D,DDS,DSQ,WKI,X0
      REAL SCALE,DEV,DEVSQ,CORNEW,XNOB,FN,SPOPN,SPOPX,SNP,SNX,SNY
      REAL SNXX,SNXY,SNYY,DN,DX,PN,PX,EDDS,P,TERM,RESLOG,WC
      REAL BPROPX,BNX,BNY,CSNXY,CSNXX,SLOP,RATIO,DIST,REGCOR,RX,RN
      REAL SIGMR1,SVAR,CORI,PVAR,TEMP,Z,BARK,BPOPX,SDPRED,VTEMP
      REAL BACHLO,BRATIO
      REAL STDRAT(MAXSP),CORTEM(MAXSP),PSIGSQ(MAXSP)
      
      CHARACTER DAT*10,TIM*8,SPEC*2,VVER*7,REV*10
C
C     PSIGSQ values for EP,AT,AC taken from WC
C
      DATA PSIGSQ/
     &  0.0408,  0.0586,  0.1556,  0.0970,  0.0858,
     &  0.1433,  0.0636,  0.0970,  0.0970,  0.0636,
     &  0.0898,  0.0898,  0.0898,  0.1556,  0.0898 /
C-----------
C  CHECK FOR DEBUG.
C-----------
...
C  TRIAL: APPLY 'MORTALITY'-TYPE CHANGES TO DIAMETER GROWTH
C  HERE, RATHER THAN AS DIRECT MORTALITY.
C  - DR: 20 DECEMBER 99
...
C----------
      ...
      COR(ISPC)= WC * CORI
C
C     CAP SCALING TERM AT 4 (LN(4) = 1.38629)
C     MARK FOR REPORTING
C
	LTEM(ISPC) = .FALSE.
	IF (COR(ISPC) .GT. 1.38629) THEN
	  COR(ISPC) = 1.38629
	  LTEM(ISPC) = .TRUE.
      ENDIF
...
C
 ...
 9006 FORMAT (/' INITIAL SCALE FACTORS FOR THE'/
     >       ' DBH INCREMENT MODEL',((T49,1X,11(F5.2,1X)/)))
C----------
...
c          DDS = EXP(DDS)
c          DDS = (DDS*DDS + 2.0*DDS*D*BRATIO(ISPC,D,0.0))*CMtoIN*CMtoIN
c          DDS = LOG(DDS)
