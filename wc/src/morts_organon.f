...
C  TREE RECORD.  MORTALITY RATE FOR LARGE TREES IS PREDICTED FROM DBH,
C  CROWN RATIO, SITE INDEX, AND BASAL AREA INTREES LARGER. IF STAND GETS
C  ABOVE MAXSDI IT WILL APPLY RATE UNTIL STAND IS BELOW MAX. IF THIS
C  ROUTINE IS CALLED FROM **TREGRO** WHEN CYCLING FOR GROWTH PREDICTION.
C  ENTRY **MORCON** IS ACCESSED TO LOAD SITE DEPENDENT CONSTANTS.
C----------
...
C----------
      ...
      REAL DGT,RELDBH,WKI,B5,D1,D2,XMORT,AVED,WPROB,DSUM
...
C----------
      ...
      XSITE1=SITEAR(16)
C----------
C IN WC CONVERT CURTIS SI TO KING SI
C----------
      CALL VARVER(VVER)
      IF (VVER(:2).EQ.'WC') XSITE1=5.21486+0.66486*SITEAR(16)
C
      SELECT CASE(MORTMAP(ISPC))
        CASE(1)
...
C----------
   55 CONTINUE
      ...
      IF(LZEIDE)THEN
        DQ10A=(SUMDR10/TA)**(1./1.605)
      ENDIF
      SDIA=TA*(DQ10A/10)**1.605
      IF(DEBUG)WRITE(JOSTND,*)'IN MORTS PASS,TA,SD2SQA,DQ10A,BAA,SDIA=',
     &PASS,TA,SD2SQA,DQ10A,BAA,SDIA
C----------
...
C----------
      IF ((SDIA.LT.SDIMAX).AND.(BAA.LT.550)) GO TO 70
C      IF ((SDIA.LT.SDIMAX).AND.(BAA.LT.SDIMAX*0.65+25)) GO TO 70
  ...
  59  CONTINUE
     ...
     &SUMDR10,TA=',PASS,I,DBH(I),WKI,WKIT2,SD2SQA,SUMDR10,TA
   60 CONTINUE
      GOTO 55
C----------
