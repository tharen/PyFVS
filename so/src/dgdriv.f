...
C----------
      ...
      INTEGER NUMCAL(MAXSP),ISI,IREFI,N
      REAL SCALE,CORNEW,DEV,DEVSQ,XNOB,FN,SPOPN,SPOPX,SNP,SNX,SNY,SNXY
      REAL SNXX,SNYY,DN,DX,PN,PX,BARK,EDDS,TERM,RESLOG,WV,BPOPX,BNX
      REAL BNY,CSNXY,CSNXX,SLOP,RATIO
      REAL PVMLT,CORR,SFINT,VARYP1,EVARP1,SIG1,VARYP2,EVARP2,SSIGMA,RHO
      REAL RHOCP,XDGROW,FRL,FRM,FRU,CORMLT,WKI,D,DDS,DSQ,BRATIO1,FRMT
      REAL P,WC,BRATIO,SDPRED,DIST,REGCOR,RX,RN,SIGMR1,SVAR,CORI,PVAR
      REAL TEMP,Z,BACHLO,VTEMP
      INTEGER I,ISPC,I1,I2,I3,ITRIPU,ITRIPL,KOUT,K,ISPEC,KK
      REAL RNPAR
      DATA PSIGSQ/
     &  0.0408,  0.0586,  0.1556,  0.0970,  0.0858,
     &  0.1433,  0.0636,  0.0970,  0.0898,  0.0636,
     &  0.0858,  0.0970,  0.0970,  0.0970,  0.0898,
     &  0.0408,  0.0586,  0.0858,  0.0898,  0.0898,
     &  0.0898,  0.0898,  0.0898,  0.1433,  0.0898,
     &  0.0898,  0.0898,  0.0898,  0.0898,  0.0898,
     &  0.0898,  0.1556,  0.0898/
C-----------
C  CHECK FOR DEBUG.
C-----------
...
C----------
      ...
      IF(DEBUG)WRITE(JOSTND,*)'I,SSIGMA,FRM,RHO,RHOCP,DSQ,DDS= '
      IF(DEBUG)WRITE(JOSTND,*)I,SSIGMA,FRM,RHO,RHOCP,DSQ,DDS
C
      DG(I)=(SQRT(DSQ+DDS*FRM)-D)
      BRATIO1=BRATIO(ISPC,DBH(I),HT(I))
      IF(DEBUG)WRITE(JOSTND,*)'DBH(I),D,DDS,DSQ,WK2(I),DG(I),BRATIO1'
      IF(DEBUG)WRITE(JOSTND,*)DBH(I),D,DDS,DSQ,WK2(I),DG(I),BRATIO1
C----------
...
C  FOR SOME SPECIES. MAKE SURE PVAR HAS THE RIGHT VALUE.
C----------
      IF(IFOR .EQ. 10)THEN
        SELECT CASE (ISPC)
        CASE (7)
