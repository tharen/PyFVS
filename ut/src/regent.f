...
C  THAT ARE LESS THAN 6 INCHES DBH AND THE DBH INCREMENT MODEL IS
C  APPLIED TO TREES THAT ARE LESS THAN 3 INCHES DBH.  FOR TREES THAT
C  ARE GREATER THAN 2 INCHES DBH (1 INCH FOR LODGEPOLE PINE), HEIGHT
C  INCREMENT PREDICTIONS ARE AVERAGED WITH THE PREDICTIONS FROM THE
C  LARGE TREE MODEL.  HEIGHT INCREMENT IS A FUNCTION OF SITE INDEX,
...
C----------
      ...
      REAL XMAX(MAXSP),XMIN(MAXSP),SLO(MAXSP),SHI(MAXSP),DAT45
      REAL AB(9),DGMAX(MAXSP),DIAM(MAXSP),P2,PS,P4,HAT3,P3
      REAL BI,CI,PN,BAYR,CCFYR,DGMX,CR,RAN,RDJ,TERM,EDH,SNY,SNX,SNP
      REAL CORNEW,SCALE3,DDS,BARK,DKK,XWT,HTGR,HK,DK,H,XPPMLT,H2
      REAL RSIMOD,RELSI,HTGRR,TPCCF,H1,D,XMN,XMX,XRDGRO,XRHGRO
      REAL CON,PPCCF,BACHLO,C2,C1,CW,B1,B2,BRATIO,D2,P,D1,SCALE
      REAL TEMCCF,TEMBA,BACON,REGYR,CORTEM(MAXSP),CRCODE
      REAL FNT,SCALE2,CCF,AVHT,X,PCTRED,SI,POTHTG,HITE1,AG2,HITE2,SJ
      REAL VIGOR,ZZRAN,X1,BKPT,BX,AX,HTNEW,HTMAX,HTMAX2
C----------
C  DATA STATEMENTS.
C----------
...
C----------
      ...
      SJ=SITEAR(ISPC)
C----------
      IF(DEBUG) WRITE(JOSTND,9981) SI,RELSI,X,ISPC,POTHTG
 9981 FORMAT('IN REGENT SI,RELSI,X,ISPC,POTHTG = ',
...
C
C----------
C  COMPUTE POTENTIAL HEIGHT GROWTH AS A FUNCTION OF SJ AND SPECIES.
C  ADJUSTMENT OF 0.83 IS SO OPEN GROWN TREES OF THESE SPECIES HIT THE
C  SITE HEIGHT ACROSS THE RANGE OF POSSIBLE SITE INDICIES
C----------
      SELECT CASE (ISPC)
C
      CASE(1:5,7:10,23)
...
C  EQUATIONS SWITCHED TO THE UT FORMULATION FOR THESE SPECIES BECAUSE
C  THEY PERFORM BETTER THAN THE CR EQUATIONS.
C----------
      CASE(17)
        POTHTG = ((SJ/5.0)*(SJ*1.5-H)/(SJ*1.5))* 0.83
C
...
C----------
      CASE(20,21)
C        POTHTG = ((1.47043 + 0.23317*SJ)/(31.56252 - 0.05586*SJ))*10.
        POTHTG = ((SJ/5.0)*(SJ*1.5-H)/(SJ*1.5))* 0.83
C
      END SELECT
C
...
C----------
C  VIGOR ADJUSTMENT TO DRASTIC FOR PINYON, JUNIPER, OAK, AND
C  OTHER HARDWOODS. CUT IT BY TWO-THIRDS
C----------
      SELECT CASE (ISPC)
      CASE(11:17,24)
        VIGOR=1.-((1.-VIGOR)/3.)
...
C
      CASE(11:16,24)
        ...
        EDH=POTHTG*PCTRED*VIGOR*RHCON(ISPC)
C----------
C  ASPEN USE EQUATIONS FROM WAYNE SHEPPARD RMRS
C----------
      CASE(6)
