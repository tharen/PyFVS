      ...
      implicit none
C----------
C  **CRATET--UT   DATE OF LAST REVISION:  09/23/11
C----------
...
C----------
      ...
      CHARACTER*4 UNDER
      INTEGER I,J,II,ISPC,IPTR,I1,I2,I3,K1,K2,K3,K4,JJ,IJ,IS,IM,NH,IDM1
      REAL AX,Q,SUMX,H,D,P,BX,XX,YY,XN,HS,P2,P3,P4,YOUNG,OLD
      INTEGER KNT2(MAXSP),KNT(MAXSP)
      REAL SPCNT(MAXSP,3)
      REAL D1,D2,SITAGE,SITHT,AGMAX,HTMAX,HTMAX2
      REAL TEMCCF,DM1,CCFT,CW,SI
C----------
...
C----------
C  GROWTH EQUATIONS FOR WB, LM, WF, BS, LP, ES, AF, PP, AND OS WERE FIT
C  USING SITE INDEX ADJUSTED TO A 50-YEAR AGE BASE. ADJUST THE SITE VALUES
C  FOR THESE SPECIES TO THIS BASIS.
C
...
C
     ...
     &    - 0.0000455*(50.0**2.0)*SI
        SITEAR(ISPC)=H
C
C SPECIES USING ALEXANDER 1967, RES NOTE RM-32
C
      CASE(4,5,8,9)
     ...
     &      (22.71944*(SI**(-0.63557)))
        SITEAR(ISPC)=H
C
C SPECIES USING MEYER 1961 rev., TECH BULL 630
C
      CASE(10)
        H = (3.635794*(SI**0.916307)) / (1.0 + EXP(6.09478 -
     &      (0.96483*ALOG(50.)) - (0.277025*ALOG(SI))))
        SITEAR(ISPC)=H
      END SELECT
...
C----------
 ...
 9005 FORMAT('HEIGHT-DIAMETER COEFFICIENTS FOR SPECIES ',I2,
     &      ':  INTERCEPT=',F10.6,'  SLOPE=',F10.6,'  IABFLG=',I4,
     & ' (9005 CRATET)')
C----------
...
C----------
      ...
      DO I=1,ITRN
      H = HT(I)
      IF(ABIRTH(I) .LE. 0.0 .OR. ABIRTH(I).GT. 999.)THEN
        SITAGE = 0.0
        SITHT = 0.0
        AGMAX = 0.0
        HTMAX = 0.0
     ...
     &              DEBUG)
      ...
      ENDIF
C----------
C  EXCLUDE SEEDLINGS FROM EA/UEA DETERMINATION. TREES EXACTLY 4.5'
C  ARE EXCLUDED BECAUSE WE HAVE THEM AS .1 DBH AND THIS WOULD
C  PROBABLY MESS UP THE CALCUALTION.  DIXON. 5-2-91.
C----------
      IF(H .GT. 4.5) THEN
        IF(ABIRTH(I).LE.1.)GO TO 210
...
C COMPUTE AGE RANGE.
C----------
      AGERNG = ABS(OLD - YOUNG)
C----------
C  CALL DGDRIV TO CALIBRATE DIAMETER GROWTH EQUATIONS.
C----------
      IF(DEBUG) WRITE(JOSTND,*)'CALL DGDRIV FROM CRATET SECOND',
     & ' TIME'
      CALL DGDRIV
C----------
