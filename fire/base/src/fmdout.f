      ...
      IMPLICIT NONE
C----------
C  $Id: fmdout.f 767 2013-04-10 22:29:22Z rhavis@msn.com $
C----------
...
C.... VARIABLE DECLARATIONS.
      ...
      INTEGER   MYACT(1), MYACT2(1), MYACT3(2)
      DIMENSION PRMS(2)
      DATA      MYACT/2515/
      DATA      MYACT2/2544/
      DATA      MYACT3/2551,2552/
      INTEGER   IYR,JROUT,ITODO,IFT,NPRM,IACTK,JYR,I,J,K,L,ISZ,JSZ,KSZ,
     &          KSP,IDC,ITM,ITON,JS,II,DBSKODE, KYR
      REAL      PRMS,TOTDUF,TOTLIT,SNVIS,SNVIH,ACRES,TOTFOL,TOTCON,D,H,
...
C     RETREIVE THE UNIT NUMBER TO BE USED FOR THE REPORT.
C
      CALL GETLUN (JROUT)
C
...
C     CHECK TO SEE WHETHER THE CARBON REPORT HAS BEEN REQUESTED.
C     (NOTE THAT IT IS NOT WRITTEN IN THIS ROUTINE, BUT IT NEEDS MANY OF
C      THE VALUE THAT ARE CALCULATED HERE.)

      CALL GETLUN (JROUT)
      CALL OPFIND(1,MYACT2,ITODO)
      IF (ITODO.GT.0) THEN
        DO I = 1,ITODO
...
C     CHECK TO SEE WHETHER THE DOWN WOOD VOLUME REPORT HAS BEEN REQUESTED.

      CALL GETLUN (JROUT)
      CALL OPFIND(1,MYACT3(1),ITODO)
      IF (ITODO.GT.0) THEN
        DO I = 1,ITODO
          CALL OPGET(I,2,KYR,IACTK,NPRM,PRMS)
...
C     CHECK TO SEE WHETHER THE DOWN WOOD COVER REPORT HAS BEEN REQUESTED.

      CALL GETLUN (JROUT)
      CALL OPFIND(1,MYACT3(2),ITODO)
      IF (ITODO.GT.0) THEN
        DO I = 1,ITODO
          CALL OPGET(I,2,KYR,IACTK,NPRM,PRMS)
...
C     PRINT FLAG TO TRUE (DEFAULT IS FALSE)
      ...
      IF (IYR .LT. IFLALB .OR. IYR .GT. IFLALE) LPRINT = .FALSE.
        IF (IFSTEP .GT. 0) THEN
          IF (MOD((IYR-IFLALB),IFSTEP).NE.0) LPRINT = .FALSE.
        ELSE
          LPRINT = .FALSE.
        ENDIF
 45   CONTINUE
...
C     DO THIS AGAIN FOR THE DOWN WOOD VOLUME AND COVER REPORTS 
      ...
      IF (IYR .LT. IDWRPB .OR. IYR .GT. IDWRPE) LPRINT2 = .FALSE.
        IF (IDWRPI .GT. 0) THEN
          IF (MOD((IYR-IDWRPB),IDWRPI).NE.0) LPRINT2 = .FALSE.
        ELSE
          LPRINT2 = .FALSE.
        ENDIF
 46   CONTINUE
      ...
      IF (IYR .LT. IDWCVB .OR. IYR .GT. IDWCVE) LPRINT3 = .FALSE.
        IF (IDWCVI .GT. 0) THEN
          IF (MOD((IYR-IDWCVB),IDWCVI).NE.0) LPRINT3 = .FALSE.
        ELSE
          LPRINT3 = .FALSE.
        ENDIF
 47   CONTINUE
