...
C     ABOVEGROUND COEFFICIENTS:
C     CEDAR/LARCH, DOUGLAS-FIR, FIR/HEMLOCK, PINE, SPRUCE
C     ASPEN/ALDER/COTTONWOOD/WILLOW, SOFT MAPLE/BIRCH, MIXED HARDWOOD,
C     HARD MAPLE/OAKHICKORY/BEECH, WOODLAND JUNIPER/OAK/MESQUITE
...
C     MERCH AND BELOWGROUND COEFFICIENTS:
C     SOFTWOOD,  HARDWOOD
...
C  CALCULATE JENKINS BIOMASS (EQUATIONS WERE FIT TO TREES GE 2.5 CM)
      ...
      IF (DCM .GT. 0.0) THEN
        ...
        ELSE
C         IF TREE IS LESS THAN 2.5 CM, THEN CALCULATE FOR A 2.5 CM TREE AND SCALE
C         BASED ON ACTUAL DIAMETER.  USE THE BELOWGROUND PROPORTION FOR A 2.5 CM TREE.
          ABIO = EXP(B0A(IGRP) + B1A(IGRP)*LOG(2.5))
          ABIO = ABIO * (DCM / 2.5)
          RBIO = ABIO * EXP(B0B(JGRP) + (B1B(JGRP) / 2.5))
        ENDIF
