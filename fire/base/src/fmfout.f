      ...
      IMPLICIT NONE
C----------
C  $Id: fmfout.f 767 2013-04-10 22:29:22Z rhavis@msn.com $
C----------
...
C
     ...
     &         TOTBAK(MXSP1),TOTVOLK(MXSP1), BurnG12
      ...
      INTEGER  MYACTS(4)
C
      DATA    MYACTS/2503,2504,2527,2528/
C
      INTEGER IYR,JROUT,NTODO,K,NPRM,IACTK,JYR,ITODO,
     &        I,J,L,II,IJ,IK,ICRB,JTODO,KSP,ICLS
      INTEGER DBSKODEM, DBSKODEB, DBSKODEF
      REAL    FLAME,PRMS(1),SUML3,SUMG3,BLIVE,TOTG3,PDUFF,PGR3,BDTOT
...
C     CHECK TO SEE IF BURN CONDITIONS REPORT HAS BEEN REQUESTED.
C     SET IFMBRE TO THE LAST YEAR THAT THE REPORT WILL BE GENERATED.
C     ALSO CALL GETID TO GET THE REPORT ID FOR THIS REPORT. RETREIVE
C     THE UNIT NUMBER FOR ANY REPORTS WHICH MAY BE WRITTEN.


      CALL GETLUN (JROUT)
C
      CALL OPFIND(1,MYACTS(2),NTODO)
      IF (NTODO .LE. 0) GO TO 21
      DO 20 K = 1,NTODO
        CALL OPGET(K,1,JYR,IACTK,NPRM,PRMS)
        IF (JYR .NE. IYR) GO TO 20
        IFMBRB = JYR
        IFMBRE = JYR + PRMS(1)
        CALL OPDONE (K,JYR)
   20 CONTINUE
...
C*************************************
C
C     CHECK TO SEE IF FUEL CONSUMPTION REPORT HAS BEEN REQUESTED.
C     SET IFMFLE TO THE LAST YEAR THAT THE REPORT WILL BE GENERATED.
C     ALSO CALL GETID TO GET THE REPORT ID FOR THIS REPORT.
C
      CALL OPFIND(1,MYACTS(3),ITODO)
      IF (ITODO .LE. 0) GO TO 110
      DO 115 K = 1,ITODO
         CALL OPGET(K,1,JYR,IACTK,NPRM,PRMS)
         IF (JYR .NE. IYR) GO TO 115
         IFMFLB = JYR
         IFMFLE = JYR + PRMS(1)
         CALL OPDONE (K,JYR)
  115 CONTINUE
...
C*************************************
C
C     CHECK TO SEE IF MORTALITY REPORT HAS BEEN REQUESTED.
C     SET IFMFLE TO THE LAST YEAR THAT THE REPORT WILL BE GENERATED.
C     ALSO CALL GETID TO GET THE REPORT ID FOR THIS REPORT.
C
      CALL OPFIND(1,MYACTS(4),JTODO)
      IF (JTODO .LE. 0) GO TO 260
      DO 255 K = 1,JTODO
        CALL OPGET(K,1,JYR,IACTK,NPRM,PRMS)
        IF (JYR .NE. IYR) GO TO 255
        IFMMRB = JYR
        IFMMRE = JYR + PRMS(1)
        CALL OPDONE (K,JYR)
  255 CONTINUE
