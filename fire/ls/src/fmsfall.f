     ...
     &                   RSOFT,RSMAL,DFALLN)
      ...
      implicit none
C----------
C  **FMSFALL--FIRE-LS  DATE OF LAST REVISION: 11/30/09
C----------
C
...
C----------
      ...
      BASE = -0.006 * D + 0.18 ! note - this is a new equation

C----------
C  In LS-FFE, fall down breakpt dbh is 18 inches (except for cedar and
C  tamarack, where it is 12 inches).
C----------

      MODRATE = BASE * FALLX(KSP)
      IF (MODRATE .LT. 0.01) MODRATE = 0.01
      IF (MODRATE .GT. 1) MODRATE = 1
      IF (D .LT. 12.0) THEN
        DFALLN = MODRATE * ORIGDEN
      ELSEIF ((D .LT. 18.0) .AND. (KSP .NE. 10) .AND.
     &        (KSP .NE. 11) .AND. (KSP .NE. 14)) THEN
        DFALLN = MODRATE * ORIGDEN
      ELSE
