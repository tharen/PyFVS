...
C
C SETTING FOR SPECIES 16 AND 19 IS FROM THE ORGANON CODE
C----------
C
...
C HEIGHT AT AGE 100) FROM DF, FROM KING CURVE (BASE AGE 50).
C----------
...
C----------
      ...
      VAR='OP'
C
C  USE SPECIAL FORESTS FOR BIA LANDS
C  QUINAULT IR (FC=800 USES OLYMIC 609), ALSO IN INITRE VOLEQNUM PROCESS
C
      IF(IREGN.EQ.8)THEN
        IRDUM=6
        FORDUM='09'
...
C----------
C  SET ORGANON VARIABLES AS APPROPRIATE
C
C  IF STAND AGE FROM STDINFO KEYWORD AND ORGANON .INP FILE IS 0
...
C
C     DON'T ADJUST THESE FOR NOW. SHOWN HERE SO WE KNOW WHAT THEY ARE
C
C     RVARS(6)	= DF GENETIC WORTH VALUE FOR DIAMETER GROWTH
C     RVARS(7)	= DF GENETIC WORTH VALUE FOR HEIGHT GROWTH
C     RVARS(8)	= N YRS FOLIAGE RETENTION FOR DF (SWISS NEEDLE CAST)
C     RVARS(9)	= PLANTING DENSITY OF ALDER RAP VERSION
C
C     RVARS(10-30)	= UNUSED.
C
