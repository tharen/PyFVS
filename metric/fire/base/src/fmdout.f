...
C
     ...
     &    LARGE12*CONV,FLIVE(1)*CONV,
     &    FLIVE(2)*CONV,TOTSUR*CONV,TOTSNG(1)*CONV,TOTSNG(2)*CONV,
     &    TOTFOL*CONV,TOTLIV(1)*CONV,NINT(TOTLIV(2)*CONV),
     &    NINT(TOTSTD*CONV),NINT(TOTFUL*CONV),NINT(TOTCON*CONV),
     &    NINT(TONREM*CONV),DBSKODE)
...
C     IT, THEN DO SO.
  ...
  720    FORMAT(1X,I5,1X,'YEAR LITT.  DUFF  <7.6  >7.6 -15.2',
     ...
     &    '  >7.6 TOTAL BIOMASS CONS REMOVED')
      ENDIF
C
     ...
     &     (CWD(3,5,1,5)+CWD(3,5,2,5))*CONV,
     &     LARGE12*CONV,
     &     (FLIVE(I)*CONV,I=1,2),TOTSUR*CONV,
     &     TOTSNG(1)*CONV,TOTSNG(2)*CONV,TOTFOL*CONV,
     &     TOTLIV(1)*CONV,NINT(TOTLIV(2)*CONV),
     &     NINT(TOTSTD*CONV),NINT(TOTFUL*CONV),NINT(TOTCON*CONV),
     &     NINT(TONREM*CONV)
C
...
C
      ...
      V1(1) = (CWDVOL(3,1,2,5)+CWDVOL(3,2,2,5)+CWDVOL(3,3,2,5))*CONV2
      V1(2) = CWDVOL(3,4,2,5)*CONV2
      V1(3) = CWDVOL(3,5,2,5)*CONV2
      V1(4) = CWDVOL(3,6,2,5)*CONV2
      V1(5) = CWDVOL(3,7,2,5)*CONV2
      V1(6) = CWDVOL(3,8,2,5)*CONV2
      V1(7) = CWDVOL(3,9,2,5)*CONV2
