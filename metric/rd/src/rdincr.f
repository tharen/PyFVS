      ...
      implicit none
C----------
C  **RDIN--CR    LAST REVISION:  06/03/10
C----------
C
C  Purpose :
C     This is the master routine for reading in root disease keywords.
C
C     Format statements that handle I/O based on number of species (MAXSP)
C     must be changed from 11 to 24 for Central Rockies variant.
C     Those format statements are:  606, 711, 1321, 2921, 3963
C
C  Called By :
C     INITRE  [PROGNOSIS]
C
...
C     10/21/97 - Added the code to set IRTSPC for the Central Rockies
C                variant.
C     06/02/98 - Robert Nathan Havis (FHTET)
...
C                Fixed additional format statments to handle I/O for 24
C                species. Keywords affected were RRHOSTS and INFKILL.
...
C  29-MAR-2005 Lance R. David (FHTET)
C     Made RD available to CR Model Type 2. The fact that it was not enabled
C     was overlooked during the processes of implementing CR 24 species and
C     RD v3.0.
...
C     RSLOP, ROWDOM, ROWIBP, RRPSWT, SSSFAC.  In the root disease model
C     the defaults for these variables are indexed as follows :
C
...
C     that species match between the FVS model and the root
C     disease model.
C
C     For the Central Rockies variant (CR) set the species mapping
C     array IRTSPC based on the model type.  In the other
C     variants of FVS IRTSPC is set in the variant dependent
C     subroutine RDBLK1
C
C     This was changed when the species list was increased to 24 species,
C     making the species consistant across all model types.
C     (RNH Dec98)
C
C  Model type 1 - Southwest mixed conifer
...
C  Model type 5 - Lodgepole pine
      ...
      IF (IMODTY .LT. 1 .OR. IMODTY .GT. 5) THEN
         WRITE(JOSTND,60)
   60    FORMAT (/,' ******** ERROR !!!   MODEL TYPE HAS NOT BEEN ',
     &           'SELECTED.  STDINFO KEYWORD IS REQUIRED BEFORE ROOT ',
     &           'DISEASE KEYWORDS.')
         STOP 20
      ENDIF

      WRITE(JOSTND,50) IMODTY
   50 FORMAT(/,9X,'   CENTRAL ROCKIES MODEL TYPE ',I1,' IS BEING USED ',
     &       'FOR THE ROOT DISEASE MODEL.')
...
C 208 FORMAT(/1X ,A8, '   STATIC CARRYOVER MODEL',
C    >     2X,'; PROB. OF NEW INFECTION CENTERS BEING FORMED=',F4.2,
C    >      ';  NEW CENTERS = ',I4,/
C    >        T13,'MODEL CALLED ',I4,' CYCLES AFTER MANAGEMENT')
C     GOTO 90
C
C 207 CONTINUE
C     IF(LKECHO)WRITE(JOSTND,210) KEYWRD,ARRAY(2),PINSET
C 210 FORMAT (/1X,A8,'*** CARRYOVER MODEL KEYWORD, PROBABILITY',
C    > ' OF INFECTION',/,
C    >      T13, '***** ASSIGNED ',F4.2,' OUT OF BOUNDS, SET TO ',F4.2)
C     GOTO 90
C
C 206 CONTINUE
C     IF(LKECHO)WRITE(JOSTND,211) KEYWRD,RRGEN(1,7),IRGEN(1)
C 211 FORMAT (/1X,A8,'   DYNAMIC CARRYOVER MODEL, MINIMUM SPREAD= ',
...
C 606    FORMAT (11F5.2)
C     Increased Format 606 columns to 24 species (RNH, Dec98)
  606    FORMAT (24F5.2)
C
      ELSE
...
C     Increased Format 711 columns to 24 species (RNH, Dec98)
  711    FORMAT (24F5.2)
C 711    FORMAT (11F5.2)
...
C.... IF TREE RECORD DATA HAS BEEN READ BEFORE ROOT DISEASE KEYWORDS
C.... HAVE BEEN PROCESSED WRITE ERROR MESSAGE AND TERMINATE RUN. THIS IS
C.... NECESSARY BECAUSE METHOD OF ROOT DISEASE INITIALIZATION MUST BE
C.... SPECIFIED IN ORDER FOR INTREE TO PROPERLY PROCESS TREE RECORDS.
...
C     to the outpu file (RNH June98)
C
...
CM  2002 FORMAT (T13,'OUTPUT WILL BE IN LOTUS READABLE FORMAT')
...
C2159 FORMAT (1X,A10,' ISL=',I4, ' ISPC=',I4, '   DEN=',F8.2,
...
C     Increased Format 2921 columns to 24 species (RNH, Dec98)
 2921    FORMAT (24F5.2)
C2921    FORMAT (11F5.2)
...
C 3000 FORMAT (/1X,'BORAX      PROP TREAT:', F4.2, '   MIN DBH:', F6.2)
...
C.... OUTPUT FOR BBTYPE4 KEYWORD
     ...
     &            PRMS(2)*INTOCM,PRMS(3)/ACTOHA, PRMS(5), PRMS(4),
     &                       (PRMS(I),I=6,8)
      ELSE
         IF(LKECHO)WRITE(JOSTND,3203) KEYWRD,IDT,KARD(2),ISPC,PRMS(2),
     &                       PRMS(3), PRMS(5), PRMS(4), (PRMS(I),I=6,8)
      ENDIF
...
C....          INVALID DISEASE TYPE
               ...
               IF(LKECHO)WRITE(JOSTND,3965) RRJSP(IRTSPC(ISPC)), IDI
               IDI = MAXRR
               IF (MAXRR .LT. 3) IDI = IDITYP(IRTSPC(ISPC))
               IF(LKECHO)WRITE(JOSTND,3966) IDI, DISTYP(IDI)
            ELSE
...
C....             INVALID DISEASE TYPE
                  ...
                  IF(LKECHO)WRITE(JOSTND,3965) RRJSP(IRTSPC(KSP)), IDI
...
C.... IN THE STAND
 ...
 4210    FORMAT (/1X,A8,3X,A15,' WILL BE SIMULATED.')
      ELSEIF (MAXRR .EQ. MINRR+1) THEN
         IF(LKECHO)WRITE(JOSTND,4215)KEYWRD,DISTYP(MINRR),DISTYP(MAXRR)
