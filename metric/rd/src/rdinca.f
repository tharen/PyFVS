      ...
      implicit none
C----------
C  **RDIN--CA    LAST REVISION:  06/03/10
C----------
C
C  Purpose :
...
C     must be changed from 11 to 49 for Inland CA S. CAscades variant.
...
C     12/18/98 - Matt Oberle (MJO DEC98) (FHTET)
C                Added 6 new species names to the RRJSP data table.
...
C                Fixed additional format statments to handle I/O for 49
...
C.... 6-BBTYPE3 7-BORATE 8-SPORE   9-BBTYPE4
     ...
     >            '       NOBLE FIR', '        NON-HOST',
     >            'PRT ORFORD CEDAR', '  SHASTA RED FIR',
     >            '    COULTER PINE', '   MONTEREY PINE',
     >            '     DIGGER PINE', '   BREWER SPRUCE' /
...
C.... OUTPUT FOR RRINIT KEYWORD
  ...
  551 FORMAT (/1X,A8,'   ROOT DISEASE IS ', A12 /
     ...
     >                         PRUN(IDI)/ACTOHA
      ...
      ELSE
         IF(LKECHO)WRITE(JOSTND,543) NCENTS(IDI)
         IF (LMTRIC) IF(LKECHO)WRITE(JOSTND,575)
         IF (.NOT. LMTRIC) IF(LKECHO)WRITE(JOSTND,576)
         IF (LNOTBK(3) .OR. LNOTBK(4)) IF(LKECHO)WRITE(JOSTND,544)
      ENDIF
  ...
  554 FORMAT (T13,'------------------------------')
     ...
     &                         PCENTS(IDI,IC,3) * FTTOM
         ELSE
            IF(LKECHO)WRITE(JOSTND,555) PCENTS(IDI,IC,1), PCENTS(IDI,IC,2),
     &                         PCENTS(IDI,IC,3)
         ENDIF
...
C....    ERROR IN READING IN SPECIES CODE
         ...
         IF(LKECHO)WRITE(JOSTND,610) KEYWRD
         GOTO 90
      ENDIF
...
C.... OUTPUT FOR INFMULT KEYWORD
  ...
  723 FORMAT (T13,41('-'))
      ...
      DO 775 IDI=IRMIN,IRMAX
         ...
         DO 724 KSP=1,MAXSP
            IF(LKECHO)WRITE(JOSTND,725) RRJSP(IRTSPC(KSP)), PNINF(IRTSPC(KSP),IDI)
  724    CONTINUE
  775 CONTINUE
...
C.... OUTPUT FOR BBTYPE3 KEYWORD
      ...
      IF (LMTRIC) THEN
         ...
         IF(LKECHO)WRITE(JOSTND,1711) KEYWRD, IDT, KARD(2), ISPC, PRMS(2)*INTOCM,
     &                      PRMS(3)/ACTOHA, PRMS(4), PRMS(5)
...
C
      ...
      IF (INT(ARRAY(3)) .GT. ITOTRR) THEN
         IF(LKECHO)WRITE(JOSTND,2960) KEYWRD, ARRAY(3),ITOTRR
         IPOINT = 0
      ENDIF
...
C.... OUTPUT FOR INOCLIFE KEYWORD
      ...
      DO 4040 IDI=IRMIN,IRMAX
         ...
         IF (LMTRIC) THEN
            IF(LKECHO)WRITE(JOSTND,4050) KEYWRD, DISTYP(IDI), DECFN(IDI,1,2),
     &                          DECFN(IDI,2,2)*FTTOM,
