      ...
      IMPLICIT NONE
C----------
C  $Id: gheads.f 767 2013-04-10 22:29:22Z rhavis@msn.com $
C----------
C
...
C
   ...
   50 FORMAT(/T42,
     ...
     >1X,122('-')/T30,'PERCENTILE POINTS IN THE'/
     >T23,'DISTRIBUTION OF STAND ATTRIBUTES BY DBH       TOTAL/HA'/
     >T10,'STAND',7X,41('-'),6X,'OF STAND',9X,
     >'DISTRIBUTION OF STAND ATTRIBUTES BY'/1X,'YEAR  ATTRIBUTES',
     >6X,'10     30     50     70     90    100',7X,'ATTRIBUTES',7X,
     >'SPECIES AND 3 USER-DEFINED SUBCLASSES'/1X,4('-'),2X,11('-'),3X,
     >6(6('-'),1X),2X,14('-'),2X,42('-')/T35,'(DBH IN CMS)')
C
   10 CONTINUE
...
C
      CALL GROHED(JOTREE)
      WRITE (JOTREE,5) NPLT,MGMID,ITITLE(1:MAX(1,ISTLNB(ITITLE)))
      CALL PPLABS (JOTREE)
      WRITE (JOTREE,81)
   81 FORMAT(
     >/1X, 126('-')
     >/T23,'ATTRIBUTES OF SELECTED SAMPLE TREES',16X,
     >        ' ADDITIONAL STAND ATTRIBUTES (BASED ON STOCKABLE AREA)',
     >/7X,65('-'),2X,53('-'),
     >/7X,'INITIAL',
     >27X,'LIVE   PAST DBH   BASAL  TREES',10X,'QUADRATIC   TREES',
     >4X,'BASAL   TOPHT (M)  CROWN'/6X,'TREES/HA',12X,'DBH    HEIGHT',
     >2X,'CROWN   GROWTH    AREA    PER    STAND   MEAN DBH    PER',
     >6X,'AREA    LARGEST   COMP'/' YEAR   %TILE  SPECIES   (CMS)',
     >'  (METRES) RATIO    (CMS)    %TILE    HA     AGE     (CMS) ',4X,
     >' HA    (SQ M/HA)   40/HA   FACTOR'/1X,'----',2X,2(7('-'),1X),
...
C
      ...
      WRITE(JOSTND,210)
 210  FORMAT(/,' ',T13,'ALPHA SPECIES - FIA CODE CROSS REFERENCE:')
      RETURN
C
      ENTRY VOLEQHEAD(JOSTND)
      WRITE(JOSTND,220)
  220 FORMAT(/,T36,'NATIONAL VOLUME ESTIMATOR LIBRARY EQUATION NUMBERS',
     &/,1X,4('SPECIES CUBIC FOOT BOARD FOOT ')/
C     & 1X,4('          EQ. NO.    EQ. NO.  ')/
     &  1X,4('------- ---------- ---------- '))
