      ...
      IMPLICIT NONE
C----------
C  $Id: initre.f 767 2013-04-10 22:29:22Z rhavis@msn.com $
C----------
C
...
C----------
      ...
      CALL GRINIT
      CALL PPEATV (LACTV)
      IF (LACTV) CALL MSTRGT
      IF (.NOT.LACTV) CALL OPINIT
...
C
  ...
  95  FORMAT(/T13,'THERE WERE',I8,' KEYWORD RECORDS PROCESSED.')
      WRITE(JOSTND,105) KEYWRD
  105 FORMAT (/1X,A8,'   PROCESS THE STAND.')
C----------
...
C----------
         CALL ERRGRO (.TRUE.,8)
         WRITE(JOSTND,106)
  106    FORMAT (T13,'(NOTREES OPTION IS AUTOMATICALLY INVOKED.)')
         LNOTRE=.TRUE.
...
C
      WRITE(JOSTND,130)
  130 FORMAT (/T50,'OPTIONS SELECTED BY DEFAULT'//1X,130('-'))
      IF (.NOT.LTRERD)WRITE(JOSTND,133) TREFMT(1:80),TREFMT(81:160)
  133 FORMAT (/' TREEFMT    ',A80/T13,A80)
      IF (BAF .LT. 0.0) THEN
        XBAF = BAF / ACRtoHA
      ELSE
        XBAF = BAF * FT2pACRtoM2pHA
      ENDIF
      WRITE(JOSTND,2010) 'DESIGN',XBAF,FPA/ACRtoHA,
     >                    BRK*INtoCM,IPTINV,NONSTK,SAMWT,GROSPC
      IF (GROSPC.NE.1.0)WRITE(JOSTND,134) GROSPC
  134 FORMAT(T13,'STAND ATTRIBUTES ARE CALCULATED PER HA OF ',
     &   'STOCKABLE AREA.  STAND STATISTICS'/
     &   T13,'IN SUMMARY TABLE ARE MULTIPLIED BY ',F5.3,
...
C
      ...
      IF(KODTYP .EQ. 0)THEN
        ARRAY(2)=0
        KARD(2)='          '
        IF(VVER(:2).NE.'SE' .AND. VVER(:2).NE.'BC')
...
C
     ...
     &   VVER(:2).EQ.'TT' .OR. VVER(:2).EQ.'WS') THEN
      WRITE (JOSTND,139) KODFOR,KARD(2),KODTYP,IAGE,ASPECT,SLOPE,
     >                   ELEV*100*FTtoM,ADJUSTL(CPVREF)
  139 FORMAT (/' STDINFO    FOREST-LOCATION CODE=',I6,
     >     '; HABITAT TYPE=',A10,' (CODE',I3,')',
     >     '; AGE=',I5,'; ASPECT AZIMUTH IN DEGREES= ',F4.0,
     >     ';',/,T13,' SLOPE= ' ,F4.0,'%',
     > '; ELEVATION (M)=',F7.1,'; REFERENCE CODE= ',A4)
      ELSEIF(VVER(:2).EQ.'AK') THEN
        WRITE (JOSTND,131) KODFOR,KODTYP,IAGE,ASPECT,SLOPE,
     >      ELEV*10.*FTtoM,ADJUSTL(CPVREF)
  ...
  135   FORMAT (/1X,A8,'   FOREST-LOCATION CODE=',I8,
     >  '; ECOLOGICAL UNIT=',A10,
     >  '; AGE=',I5,'; ASPECT AZIMUTH IN DEGREES= ',F4.0,
     >   '; SLOPE= ',F4.0,'%'/
     > T13,'ELEVATION (M)=',F7.1,'; REFERENCE CODE= ',A4)
      ELSE
        WRITE (JOSTND,138) KODFOR,KARD(2),KODTYP,IAGE,ASPECT,SLOPE,
     >    ELEV*100.0*FTtoM,ADJUSTL(CPVREF)
  138   FORMAT (/' STDINFO    FOREST-LOCATION CODE=',I6,
     >   '; HABITAT TYPE=',A10,' (CODE',I3,')',
     >   '; AGE=',I5,'; ASPECT AZIMUTH IN DEGREES= ',F4.0,
     >   '; SLOPE= ' ,F4.0,'%'/
     > T13,'ELEVATION (M)=',F7.1,'; REFERENCE CODE= ',A4)
...
C----------
 ...
 210     FORMAT(/1X,A8,'   SITE INDEX INFORMATION:')
         WRITE(JOSTND,211) (NSP(I,1)(1:2),FTtoM*SITEAR(I),I=1,MAXSP)
 211     FORMAT ((T13,8(A3,'=',F6.1,:,'; '),A,'=',F6.1))
         IF(ISISP.GT.0)THEN
           WRITE(JOSTND,212) NSP(ISISP,1)(1:2),ISISP
 212       FORMAT(T13,'SITE SPECIES=',A,' CODE=',I5)
         ELSE
           WRITE(JOSTND,213) ISISP
 213       FORMAT(T13,'SITE SPECIES NOT SPECIFIED.  CODE=',I5)
        ENDIF
        IF(VVER(:2).EQ.'NI' .OR. VVER(:2).EQ.'CI'
     &     .OR. VVER(:2).EQ.'KT' .OR. VVER(:2).EQ.'IB') THEN
          WRITE(JOSTND,214)
 214      FORMAT(T13,'NOTE: SITE INDEX IS NOT USED IN THIS VARIANT.',
     &    '  VALUES ARE PRINTED FOR INFORMATIONAL PURPOSES.' )
        ENDIF
        IF(NSITET.GT.0)THEN
          WRITE(JOSTND,2141)
 2141     FORMAT(/,T13,'SITE INDEX TREE INPUT SUMMARY TABLE:',/,
     &    T13,'SPECIES',2X,'DIAMETER',4X,'HEIGHT',6X,'AGE',5X,'AGE REF',
     &    4X,'PLOT REF',/,T13,'-------',2X,'--------',4X,'------',6X,
     &    '---',5X,'-------',4X,'--------')
          DO 2143 I=1,NSITET
          IAGERF='        '
          IPLTRF='        '
          ...
          IISP=SITETR(I,1)
          WRITE(JOSTND,2142)IISP,NSP(IISP,1)(1:2),
     &      SITETR(I,2) * INtoCM,
     &      SITETR(I,3) * FTtoM,
     &      SITETR(I,4),
     &      IAGERF,IPLTRF   
 2142     FORMAT(T12,I3,' ('A2,')',2F10.1,F10.0,3X,A8,2X,A8)   
 2143     CONTINUE
        ENDIF
      ENDIF
C----------
...
C
 ...
 1210 CONTINUE
      IY(I) = I2
      I=I-1
      WRITE(JOSTND,1215) KEYWRD,I,I2
 1215 FORMAT(/1X,A8,3X,'CYCLE=',I3,'; PERIOD LENGTH=',I3)
      GO TO 10
 1220 CONTINUE
      DO 1230 I=2,MAXCY1
      IY(I) = I2
 ...
 1235 FORMAT(/1X,A8,3X,'ALL CYCLES;  PERIOD LENGTH=',I3)
      GO TO 10
C
...
C
     ...
     &                ARRAY,KARD)
         ...
         IF(.NOT.LNOTBK(5))ARRAY(5)=999.*CMtoIN
         ILEN=3
         IF(IS.LT.0)ILEN=ISPGRP(-IS,52)
         IF(LKECHO)WRITE(JOSTND,1330) KEYWRD,IDT,
     >      KARD(2)(1:ILEN),IS,ARRAY(3),ARRAY(4)*INtoCM,
     >      ARRAY(5)*INtoCM
 1330    FORMAT (/1X,A8,'   DATE/CYCLE=',I5,'; SPECIES=',A,' (CODE=',
     >   I3,'); CROWN WIDTH MULTIPLIER=',F10.4/
     >   T13,'ONLY TREES GREATER THAN OR EQUAL TO ',F7.2,
     >   ' AND LESS THAN ',
     >   F7.2,' CM DBH ARE AFFECTED.')
...
C
 1400 CONTINUE
      ...
      I=0
      IF (LNOTBK(2)) I=1
      IF (LNOTBK(3)) SDLO=ARRAY(3)*CMtoIN
      IF (LNOTBK(4)) SDHI=ARRAY(4)*CMtoIN
C
C     SPECIES CODE PROCESSING.
C
      IS = 0
...
C
 ...
 1410 FORMAT (/1X,A8,'   DATA SET REFERENCE NUMBER=',I3)
      IF(IS.NE.0) THEN
        ILEN=3
        IF(IS.LT.0)ILEN=ISPGRP(-IS,52)
        IF(LKECHO .AND. LNOTBK(5))
     >    WRITE(JOSTND,1411) 'SPECIES= ',KARD(5)(1:ILEN),IS,'IS'
 1411   FORMAT (T13,A,A,' (CODE= ',I3,') ',A,
     >           ' TARGETED FOR SCREENING.')
      ELSE
         IF(LKECHO .AND. LNOTBK(5))
     >    WRITE(JOSTND,1411) 'ALL',' SPECIES',IS,'ARE'
      ENDIF
      IF((LNOTBK(3) .OR. LNOTBK(4)).AND.LKECHO)
     >  WRITE(JOSTND,1405) SDLO*INtoCM,SDHI*INtoCM
 1405 FORMAT(T13,'ONLY RECORDS WITH DIAMETERS GE',F6.1,' CM OR ',
     &  'LT',F6.1,' CM WILL BE READ FROM THE DATA.',
     &  /,T13,'ALL OTHER INPUT RECORDS WILL BE SCREENED OUT.')
...
C
 ...
 1610 FORMAT (/1X,A8,'   MOUNTAIN PINE BEETLE OPTIONS:')
...
C
 ...
 1710 FORMAT (/1X,A8,'   DOUGLAS-FIR TUSSOCK MOTH OPTIONS:')
      CALL DFTMIN(LKECHO)
      GO TO 10
C
...
C
 ...
 1810 FORMAT (/1X,A8,'   WESTERN SPRUCE BUDWORM OPTIONS:')
C
...
C
 ...
 1908   FORMAT(/1X,A8,T13,'DEFAULT COEFFICIENTS FOR CROWN WIDTH',
...
C----------
      ELSE
        ...
        ILEN=3
        IF(LKECHO)WRITE(JOSTND,1918) KEYWRD,KARD(1)(1:ILEN),ISPC
 1918   FORMAT(/1X,A8,T13,'DEFAULT COEFFICIENTS FOR CROWN WIDTH',
     &    ' EQUATIONS REPLACED FOR SPECIES= ',A,' (CODE= ',I3,').' )
      ENDIF
      IF(LKECHO)WRITE(JOSTND,1925) TC*INtoCM,(WK6(I),I=1,8)
 1925 FORMAT(T13,'TRANSITION SIZE = ',F10.2,' CM DBH.',
     &  '   EQUATION FORM: C0 + C1*DBH +C2*DBH**C3'/
     &  T13,'COEFFICIENTS FOR TREES THAT ARE SMALLER THAN',
     &  ' TRANSITION SIZE:'/T13,' C0= ',F12.5,'  C1= ',F12.5,'  C2= ',
     &  F12.5,'  C3= ',F12.5,/
     &        T13,'COEFFICIENTS FOR TREES THAT ARE LARGER THAN',
     &  ' TRANSITION SIZE:'/T13,' C0= ',F12.5,'  C1= ',F12.5,'  C2= ',
     &  F12.5,'  C3= ',F12.5)
      GO TO 10
C
...
C
 2000 CONTINUE
      IF (ARRAY(1) .LT. 0.0) THEN
        ARRAY(1) = ARRAY(1) / HAtoACR
      ELSE
        ARRAY(1) = ARRAY(1) * M2pHAtoFT2pACR
      ENDIF
      ARRAY(2) = ARRAY(2) / HAtoACR
      ARRAY(3) = ARRAY(3) * CMtoIN
     ...
     >                                FLOAT(IPTINV)
      ...
      ELSE
        XBAF = BAF*FT2pACRtoM2pHA
      ENDIF
      IF(LKECHO)WRITE(JOSTND,2010) KEYWRD,XBAF,FPA/ACRtoHA,BRK*INtoCM,
     >                    MAX(0,IPTINV),MAX(0,NONSTK),MAX(0.,SAMWT),XXG
 2010 FORMAT (/1X,A,T13,'BASAL AREA FACTOR= ',F6.1,
     >        '; INVERSE OF FIXED PLOT AREA= ',F6.1,
     >        '; BREAK DBH= ',F6.1/T13,'NUMBER OF PLOTS= ',I5,
...
C
 ...
 2120 FORMAT (/1X,A8,'   NUMBER OF CYCLES=',I3)
      GO TO 10
C
...
C
 2200 CONTINUE
      ...
      IF(LKECHO)WRITE(JOSTND,2210) KEYWRD,TFPA*ACRtoHA
 2210 FORMAT (/1X,A8,'   TOTAL FIXED PLOT AREA=',F10.3)
      GO TO 10
C
...
C
 ...
 2310 FORMAT (/1X,A8,'   DIAMETER GROWTH DATA TYPE CODE= ',I1,
     >        ', ',I3,' YEAR MEASUREMENT PERIOD;' /
     >        1X,T13,'HEIGHT GROWTH DATA TYPE CODE  = ',I1,
     >        ', ',I3,' YEAR MEASUREMENT PERIOD;'/
     >        T13,'MORTALITY MEASUREMENT PERIOD  =',I2)
      GO TO 10
C
...
C
 2400 CONTINUE
      ...
      IF ( LNOTBK(7) )WRITE(CPVREF,'(I10)')IFIX(ARRAY(7))
     ...
     >     IAGE,ASPECT,SLOPE,ELEV*100.*FTtoM,ADJUSTL(CPVREF)
 2411   FORMAT (/1X,A8,'   FOREST-LOCATION CODE=',I6,
     >     '; HABITAT TYPE=',A10,' (CODE ',I3,')',
     >  '; AGE=',I5,'; ASPECT AZIMUTH IN DEGREES= ',F4.0,
     >       ';',/,T13,' SLOPE= ' ,F4.0,'%',
     > ';  ELEVATION (M)=',F7.1,'; REFERENCE CODE= ',A4)
     ...
     >     IAGE,ASPECT,SLOPE,ELEV*10.*FTtoM,ADJUSTL(CPVREF)
 2412   FORMAT (/1X,A8,'   FOREST-LOCATION CODE=',I8,
     >  '; HABITAT TYPE=',I3,
     >  '; AGE=',I5,'; ASPECT AZIMUTH IN DEGREES= ',F4.0,
     >   '; SLOPE= ',F4.0,'%'/
     > T13,'ELEVATION (M)=',F7.1,'; REFERENCE CODE= ',A4)
C
      ELSEIF (VVER(:2) .EQ. 'SN') THEN
        WRITE (JOSTND,2413) KEYWRD,KODFOR,PCOM,
     >     IAGE,ASPECT,SLOPE,ELEV*100.*FTtoM,ADJUSTL(CPVREF)
 2413   FORMAT (/1X,A8,'   FOREST-LOCATION CODE=',I8,
     >  '; ECOLOGICAL UNIT=',A10,
     >  '; AGE=',I5,'; ASPECT AZIMUTH IN DEGREES= ',F4.0,
     >   '; SLOPE= ',F4.0,'%'/
     > T13,'ELEVATION (M)=',F7.1,'; REFERENCE CODE= ',A4)
...
C
 ...
 2510 FORMAT (T13,'STAND ID= ',A,8X,A)
      GO TO 10
C
...
C
 ...
 2610 FORMAT (/1X,A8,'   INVENTORY YEAR=',I5)
      GO TO 10
C
...
C
 ...
 2710 FORMAT (/1X,A8,'   DATE/CYCLE=',I5,'; DATA SET REFERENCE ',
     >        'NUMBER =',F4.0,'; HEADING SUPPRESSION CODE =',F3.0/
     >        T13,'(0=WITH HEADING, OTHER VALUES=SUPPRESS HEADING).')
      IF((NP.GE.3.AND. ARRAY(4).EQ.1.).AND.LKECHO)WRITE(JOSTND,2720)
 2720 FORMAT (T13,'CYCLE ZERO TREELIST SUPPRESSED.')
      IF((NP.GE.3.AND. ARRAY(4).EQ.2.).AND.LKECHO)WRITE(JOSTND,2721)
 2721 FORMAT (T13,'CYCLE ONE TREELIST SUPPRESSED.')
      IF((ARRAY(5).GT.0).AND.LKECHO)WRITE(JOSTND,2740)
 2740 FORMAT(T13,'TREELIST WILL REPORT DEAD TREE STATISTICS.')
      IF((NP.EQ.6).AND.LKECHO)WRITE(JOSTND,2745)
 2745 FORMAT(T13,'ESTIMATED DIAMETER GROWTHS WILL BE PRINTED ON THE',
...
C
 ...
 2910 FORMAT (/1X,A8,'   NO SUMMARY OUTPUT WILL BE WRITTEN.')
      LSUMRY = .FALSE.
      GO TO 10
C
...
C
 ...
 3010   FORMAT (/1X,A8,'   DEBUG, ALL CYCLES. ')
      ELSE
        IF(LKECHO)WRITE(JOSTND,3020) KEYWRD,IDBCYC
 3020   FORMAT (/1X,A8,'   DEBUG, CYCLE=',I2,'. ')
      ENDIF
      IF (LNOTBK(2)) THEN
...
C
 ...
 3110 FORMAT (/1X,A8,'   SUMMARY OUTPUT WILL BE WRITTEN TO ',
...
C
 ...
 3310 FORMAT (/1X,A8,'   DATE/CYCLE=',I5,'; MIN=',F8.1,'; MAX=',
...
C
 3600 CONTINUE
      ARRAY(2)=ARRAY(2)*M2pHAtoFT2pACR
      ARRAY(4)=ARRAY(4)*CMtoIN
      ARRAY(5)=ARRAY(5)*CMtoIN
      ARRAY(6)=ARRAY(6)*MtoFT
      ARRAY(7)=ARRAY(7)*MtoFT
...
C
 ...
 3810 FORMAT (/1X,A8,'   DATE/CYCLE=',I5,
     >        '; PROPORTION OF SELECTED TREES REMOVED=',F6.3)
      IF((I.EQ.2).AND.LKECHO)WRITE(JOSTND,3820) IFIX(PRMS(2))
 3820 FORMAT (T13,'TREES CODED ',I4,' ARE SELECTED.')
      GO TO 10
C
C  ==========  OPTION NUMBER 29: THINDBH  ===========================THINDBH
C
 3900 CONTINUE
      ARRAY(2)=ARRAY(2)*CMtoIN
      ARRAY(3)=ARRAY(3)*CMtoIN
      ARRAY(6)=ARRAY(6)/HAtoACR
      ARRAY(7)=ARRAY(7)*M2pHAtoFT2pACR
      ICFLAG=228
 3901 CONTINUE
      IDT=1
      IF (LNOTBK(1)) IDT=IFIX(ARRAY(1))
...
C
     ...
     >                    ARRAY(4)
      ELSE
        IF(LKECHO)WRITE(JOSTND,3910)KEYWRD,IDT,
     >                    'HEIGHT=',ARRAY(2)*FTtoM,
     >                    'HEIGHT=',ARRAY(3)*FTtoM,
     >                    ARRAY(4)
      ENDIF
 3910 FORMAT(/1X,A8,'   DATE/CYCLE=',I5,'; MINIMUM ',A,F8.1,
     >        '; MAXIMUM ',A,F8.1,
     >        '; PROPORTION OF SELECTED TREES REMOVED=',F6.3)
      IF (IS.NE.0) THEN
         ILEN=3
         IF(IS.LT.0)ILEN=ISPGRP(-IS,52)
         IF(LKECHO)WRITE(JOSTND,3920) 'SPECIES= ',
     >     KARD(5)(1:ILEN),IS,'IS'
 3920    FORMAT (T13,A,A,' (CODE= ',I3,') ',A,
     >           ' TARGETED FOR THIS CUT.')
      ELSE
         IF(LKECHO)WRITE(JOSTND,3920) 'ALL',' SPECIES',IS,'ARE'
      ENDIF
      IF(((ARRAY(6).GT.0.0).OR.(ARRAY(7).GT.0.0))
     >  .AND.LKECHO)WRITE(JOSTND,3930) ARRAY(6)/ACRtoHA,
     &  ARRAY(7)*FT2pACRtoM2pHA
 3930 FORMAT(T13,'RESIDUAL CLASS TPH =',F8.1,
...
C
 4040 CONTINUE
      ARRAY(2)=ARRAY(2)*KGtoLB/HAtoACR
      ARRAY(3)=ARRAY(3)*KGtoLB/HAtoACR
      ARRAY(4)=ARRAY(4)*KGtoLB/HAtoACR
      CALL FFIN (JOSTND,IRECNT,KEYWRD,ARRAY,LNOTBK,KARD,LKECHO)
      GO TO 10
C
...
C
 ...
 4062 FORMAT(/1X,A8,'   STATISTICAL DESCRIPTION OF INPUT DATA WILL',
     &   ' BE PROVIDED; CONFIDENCE LIMITS AT ',F4.2,' LEVEL.')
      GO TO 10
 4068 CONTINUE
      ALPHA=0.05
      IF(LKECHO)WRITE(JOSTND,4069)
 4069 FORMAT(12X,'SPECIFIED PROBABILITY LEVEL IS INVALID; 0.05 WILL',
     &  ' BE USED.')
      GO TO 4067
C
...
C
     ...
     >      ARRAY(4)*FTtoM,
     >      KEYWRD,ARRAY(5)
 4082    FORMAT (/1X,A8,'   DATE/CYCLE=',I5,'; SPECIES= ',A,' (CODE= ',
     >      I3,'); LOWER TREE-HEIGHT LIMIT=',F5.1,'; UPPER LIMIT=',F5.1/
     >      T13,'PROBABILITY (',A8,') FOR TREES WITHIN ',
     ...
     >     ARRAY(6),ARRAY(7)
 4083    FORMAT (T13,'AVERAGE PROPORTION OF ',A,' LOST=',F6.3,
     >           '; STD DEV=',F6.3)
      ENDIF
      GOTO 10
C
...
C
      ...
      IF (KODE.GT.0) GOTO 10
      IF (ICFLAG .EQ. 223 .OR. ICFLAG .EQ. 224) THEN
     ...
     >    PRMS(6)*FTtoM
      ENDIF
 4095 FORMAT (/1X,A8,'   DATE/CYCLE=',I5,'; RESIDUAL=',F8.2,
     >        '; PROPORTION OF SELECTED TREES REMOVED=',F6.3/
...
C
 ...
 4110   FORMAT(1X,T13,'DEFECT CORRECTIONS WILL BE APPLIED TO ',
     &  'GROSS TOTAL VOLUME.')
      ELSE
        IF(LKECHO)WRITE(JOSTND,4120)
 4120   FORMAT(1X,T13,'DEFECT CORRECTIONS WILL BE APPLIED TO ',
     &  'MERCHANTABLE CUBIC METRE VOLUME.')
      ENDIF
      GO TO 10
C
...
C
 ...
 4210   FORMAT(1X,T13,'DEFECT CORRECTIONS WILL BE APPLIED TO ',
     &  'SAWLOG VOLUME.')
      ELSE
        IF(LKECHO)WRITE(JOSTND,4220)
 4220   FORMAT(1X,T13,'DEFECT CORRECTIONS WILL BE APPLIED TO ',
     &  'BOARD FOOT VOLUME.')
      ENDIF
      GO TO 10
C
...
C----------
 ...
 4321   CONTINUE
     ...
     &   VVER(:2).EQ.'SE' .OR. VVER(:2).EQ.'SN' .OR.
     &   VVER(:2).EQ.'ON') THEN
          IF(LKECHO)WRITE(JOSTND,4332) KEYWRD,KARD(2)(1:ILEN),
     >      IS,(ARRAY(I),I=3,7)
        ELSE
          IF(LKECHO)WRITE(JOSTND,4340) KEYWRD,KARD(2)(1:ILEN),
     >      IS,(ARRAY(I),I=3,7)
        ENDIF
        GO TO 10
C----------
...
C----------
 ...
 4312   FORMAT(/1X,A8,T13,'PULPWOOD VOLUME DEFECT',
     >       ' PROPORTIONS HAVE BEEN CHANGED FOR ALL SPECIES:',
     >       /T13,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2,
     >       '; 38.1 CM TREES=',F6.2,/T13,'50.8 CM TREES=',F6.2,
     >       '; 63.5 CM AND LARGER TREES=',F6.2)
      ELSE
        IF(LKECHO)WRITE(JOSTND,4320) KEYWRD,(ARRAY(I),I=3,7)
 4320   FORMAT(/1X,A8,T13,'CUBIC METRE VOLUME DEFECT',
     >       ' PROPORTIONS HAVE BEEN CHANGED FOR ALL SPECIES:',
     >       /T13,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2,
     >       '; 38.1 CM TREES=',F6.2,/T13,'50.8 CM TREES=',F6.2,
     >       '; 63.5 CM AND LARGER TREES=',F6.2)
...
C----------
     ...
     >      (ARRAY(I),I=3,7)
 4332     FORMAT(/1X,A8,T13,'GROSS TOTAL VOLUME DEFECT PROPORTIONS ',
     ...
     >    (ARRAY(I),I=3,7)
 4340   FORMAT(/1X,A8,T13,'CUBIC METRE VOLUME DEFECT PROPORTIONS HAVE ',
...
C----------
 ...
 4412     FORMAT(/1X,A8,T13,'SAWLOG VOLUME DEFECT',
     &      ' PROPORTIONS HAVE BEEN CHANGED FOR ALL SPECIES:',
     >       /T13,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2,
     >       '; 38.1 CM TREES=',F6.2,/T13,'50.8 CM TREES=',F6.2,
     >       '; 63.5 CM AND LARGER TREES=',F6.2)
 ...
 4420     FORMAT(/1X,A8,T13,'BOARD FOOT VOLUME DEFECT',
     &      ' PROPORTIONS HAVE BEEN CHANGED FOR ALL SPECIES:',
     >       /T13,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2,
     >       '; 38.1 CM TREES=',F6.2,/T13,'50.8 CM TREES=',F6.2,
     >       '; 63.5 CM AND LARGER TREES=',F6.2)
...
C----------
     ...
     &    VVER(:2).EQ.'ON') THEN
          ...
          IF(LKECHO)WRITE(JOSTND,4432) KEYWRD,KARD(2)(1:ILEN),IS,
     >      (ARRAY(I),I=3,7)
 4432     FORMAT(/1X,A8,T13,'SAWLOG VOLUME DEFECT ',
     &      'PROPORTIONS HAVE BEEN CHANGED FOR SPECIES:',A,
     &      ' (CODE=',I3,')',
     &       /T13,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2,
     &       '; 38.1 CM TREES=',F6.2,/T13,'50.8 CM TREES=',F6.2,
     &       '; 63.5 CM AND LARGER TREES=',F6.2)
        ELSE
          ...
          IF(LKECHO)WRITE(JOSTND,4440) KEYWRD,KARD(2)(1:ILEN),IS,
     >      (ARRAY(I),I=3,7)
 4440     FORMAT(/1X,A8,T13,'BOARD FOOT VOLUME DEFECT PROPORTIONS ',
     &      'HAVE BEEN CHANGED FOR SPECIES:',A,' (CODE=',I3,')',
     &       /T13,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2,
     &       '; 38.1 CM TREES=',F6.2,/T13,'50.8 CM TREES=',F6.2,
     &       '; 63.5 CM AND LARGER TREES=',F6.2)
...
C
 4500 CONTINUE
      ...
      ARRAY(3) = ARRAY(3)*CMtoIN
      ARRAY(4) = ARRAY(4)*CMtoIN
      ARRAY(5) = ARRAY(5)*CMtoFT
C
C  SET ITOPD8=1 OR ITOPD9=1 TO DISSALLOW MODIFICATION OF TOP MERCH. SPEC.
C  IN R8 AND R9 VARIANTS RESPECTIVELY
C
      ITOPD8=0
      ITOPD9=0
      IF(VVER(:2).EQ.'SN'.OR.VVER(:2).EQ.'SE')ITOPD8=1
      IF(VVER(:2).EQ.'CS'.OR.VVER(:2).EQ.'LS'.OR.
     &  VVER(:2).EQ.'NE'.OR.VVER(:2).EQ.'ON')ITOPD9=0  ! special for LS
C
C     SPECIES CODE PROCESSING.
C
      CALL SPDECD (2,IS,NSP(1,1),JOSTND,IRECNT,KEYWRD,
...
C----------
      IF(IS .LT. 0) THEN
        ...
        IF (LNOTBK(7)) METHC(IGSP) =ARRAY(7)
        ILEN=ISPGRP(IGRP,52)
        IF (VVER(:2) .EQ. 'AK') THEN
          IF(LKECHO)WRITE(JOSTND,4534) KEYWRD,KARD(2)(1:ILEN),IGSP,
     &      DBHMIN(IGSP),TOPD(IGSP),STMP(IGSP),FRMCLS(IGSP)
        ELSEIF(ITOPD8.GT.0)THEN
          WRITE(JOSTND,4537) KEYWRD,KARD(2)(1:ILEN),
     &       IGSP,DBHMIN(IGSP)*INtoCM,STMP(IGSP)*FTtoCM,
     &       FRMCLS(IGSP)
        ELSEIF(ITOPD9.GT.0)THEN
          WRITE(JOSTND,4538) KEYWRD,KARD(2)(1:ILEN),
     &       IGSP,DBHMIN(IGSP)*INtoCM,STMP(IGSP)*FTtoCM,
     &       FRMCLS(IGSP)
        ELSE
          IF(LKECHO)WRITE(JOSTND,4535) KEYWRD,KARD(2)(1:ILEN),IGSP,
     &      DBHMIN(IGSP),TOPD(IGSP)*INtoCM,STMP(IGSP)*FTtoCM,
     &      FRMCLS(IGSP)
        ENDIF
...
C----------
      ELSEIF(IS .EQ. 0)THEN
        DO 4510 IS=1,MAXSP
          ...
          IF (LNOTBK(4).AND.(ITOPD8.LE.0.AND.ITOPD9.LE.0))
     &      TOPD(IS)=ARRAY(4)
          IF (LNOTBK(5)) STMP(IS)  =ARRAY(5)
          IF (LNOTBK(6)) FRMCLS(IS)=ARRAY(6)
          IF (LNOTBK(7)) METHC(IS) =ARRAY(7)
 4510   CONTINUE
        IF(LKECHO)WRITE(JOSTND,4520) KEYWRD
 4520   FORMAT (/1X,A8,'   THE FOLLOWING MERCHANTABILITY ',
     >    'STANDARDS HAVE BEEN CHANGED FOR ALL SPECIES:')
        IF(LNOTBK(3).AND.LKECHO)WRITE(JOSTND,4521) ARRAY(3)*INtoCM
        IF (LNOTBK(4).AND.(ITOPD8.GT.0))THEN
          WRITE(JOSTND,4527)
        ELSEIF(LNOTBK(4).AND.(ITOPD9.GT.0))THEN
          WRITE(JOSTND,4528)
        ELSEIF(LNOTBK(4).AND.LKECHO)THEN
          WRITE(JOSTND,4522) ARRAY(4)*INtoCM
        ENDIF
        IF(LNOTBK(5).AND.LKECHO)WRITE(JOSTND,4523) ARRAY(5)*FTtoCM
        IF (LNOTBK(6)) THEN
          IF(VVER(:2) .EQ. 'AK') THEN
 ...
 4521   FORMAT (T23,'MINIMUM DBH =',F6.2)
 4522   FORMAT (T23,'TOP DIAMETER=',F6.2)
 4523   FORMAT (T23,'STUMP HEIGHT=',F6.2)
 4524   FORMAT (T23,'FORM CLASS  =',F6.2)
 4525   FORMAT (T23,'METHOD OF VOLUME CALCULATION IS ',F5.0)
 4526   FORMAT (T23,'LOG LENGTH  =',F6.2)
 4527   FORMAT (T23,'TOP DIAMETER CAN NOT BE MODIFIED. IT WILL BE ',
     &   'SET TO DEFAULT VALUE')
 4528   FORMAT (T23,'TOP DIAMETER CAN NOT BE MODIFIED. IT WILL BE ',
     &   'SET TO DEFAULT VALUE BASED ON NATIONAL FOREST AND SPECIES')
        GOTO 10
      ELSE
C----------
C     SPECIES CODE IS SPECIFIED (IS>0).
C----------
 ...
 4534     FORMAT (/1X,A8,'   MERCHANTABILITY STANDARDS FOR ',
     >      'SPECIES= ',A,' (CODE=',I3,') ARE:  MINIMUM DBH=',F6.2,
     >      '; TOP DIAMETER=',F6.2,'; STUMP HEIGHT=',F6.2,/T13,
     >      'LOG LENGTH=',F6.2)
     ...
     &      IS,DBHMIN(IS)*INtoCM,STMP(IS)*FTtoCM,FRMCLS(IS)           
        ELSEIF(LNOTBK(4).AND.(ITOPD9.GT.0))THEN
          IF(LKECHO)WRITE(JOSTND,4538) KEYWRD,KARD(2)(1:ILEN),
     &      IS,DBHMIN(IS)*INtoCM,STMP(IS),FRMCLS(IS)           
        ELSE
     ...
     &      STMP(IS)*FTtoCM,
     &      FRMCLS(IS)
 4535     FORMAT (/1X,A8,'   MERCHANTABILITY STANDARDS FOR ',
     >      'SPECIES= ',A,' (CODE=',I3,') ARE:  MINIMUM DBH=',F6.2,
     >      '; TOP DIAMETER=',F6.2,'; STUMP HEIGHT=',F6.2,/T13,
     >      'FORM CLASS=',F6.2)
        ENDIF
        IF(LNOTBK(7).AND.LKECHO)WRITE(JOSTND,4536)METHC(IS)
 4536   FORMAT (T13,'METHOD OF VOLUME CALCULATION IS ',I6)
 4537   FORMAT (/1X,A8,'   MERCHANTABILITY STANDARDS FOR ',
     >        'SPECIES= ',A,' (CODE=',I3,') ARE:  MINIMUM DBH=',F6.2,
     >        '; TOP DIAMETER'/,T13,'CAN NOT BE MODIFIED. IT WILL BE ',
     >        'SET TO DEFAULT VALUE; STUMP HEIGHT=',F6.2,
     >        ' FORM CLASS=',F6.2)
 4538   FORMAT (/1X,A8,'   MERCHANTABILITY STANDARDS FOR ',
     >        'SPECIES= ',A,' (CODE=',I3,') ARE:  MINIMUM DBH=',F6.2,
     >        '; TOP DIAMETER'/,T13,'CAN NOT BE MODIFIED. IT WILL BE ',
     >        'SET TO DEFAULT VALUE BASED ON NATIONAL FOREST AND ',
     >        'SPECIES'/T13,'STUMP HEIGHT=',F6.2,' FORM CLASS=',F6.2)
        GOTO 10
	ENDIF
C
...
C----------
      IF(IS .LT. 0) THEN
        ...
        IF (LNOTBK(4).AND.(ITOPD8.GT.0.OR.ITOPD9.GT.0))
     &    ARRAY(4)=TOPD(IFSP)
     ...
     >      ARRAY(3)*INtoCM, ARRAY(4)*INtoCM, ARRAY(5)*FTtoCM,
     >      (ARRAY(I),I=6,7)
        ELSEIF(ITOPD8.GT.0)THEN
          WRITE(JOSTND,4547) KEYWRD,IDT,KARD(2)(1:ILEN),IS,
     >      ARRAY(3)*INtoCM, ARRAY(5)*FTtoCM,
     >      (ARRAY(I),I=6,7)
        ELSEIF(ITOPD9.GT.0)THEN
          WRITE(JOSTND,4548) KEYWRD,IDT,KARD(2)(1:ILEN),IS,
     >      ARRAY(3)*INtoCM, ARRAY(5)*FTtoCM,
     >      (ARRAY(I),I=6,7)
        ELSE
...
C----------
 ...
 4574     FORMAT(/1X,A8,'   DATE/CYCLE=',I5,'; ALL SPECIES (CODE= 0)',
     ...
     >      '; STUMP HEIGHT=',F6.2,/T13,'LOG LENGTH=',F6.2,
     ...
     >      ARRAY(3)*INtoCM, ARRAY(5)*FTtoCM,(ARRAY(I),I=6,7)          
        ELSE
          IF(LKECHO)WRITE(JOSTND,4575) KEYWRD,IDT,
     >      ARRAY(3)*INtoCM,
     >      ARRAY(4)*INtoCM,
     >      ARRAY(5)*FTtoCM,
     >      ARRAY(6),ARRAY(7)
 4575   FORMAT(/1X,A8,'   DATE/CYCLE=',I5,'; ALL SPECIES (CODE= 0)',
     >          '; MINIMUM DBH=',F6.2,'; TOP DIAMETER=',F6.2,
...
C----------
     ...
     &    ARRAY(4)=TOPD(IS)
        ...
        IF(VVER(:2) .EQ. 'AK') THEN
          IF(LKECHO)WRITE(JOSTND,4544) KEYWRD,IDT,KARD(2)(1:ILEN),IS,
     >      (ARRAY(I),I=3,7)
 4544     FORMAT(/1X,A8,'   DATE/CYCLE=',I5,'; SPECIES= ',A,
     >      ' (CODE= ',I3,
     >      '); MINIMUM DBH=',F6.2,'; TOP DIAMETER=',F6.2,
     >      '; STUMP HEIGHT=',F6.2,/T13,'LOG LENGTH=',F6.2,
     >      ';  METH OF VOL CALC=',F6.0)
     ...
     >      ARRAY(5)*FTtoCM,
     >      ARRAY(6),ARRAY(7)
 4545     FORMAT(/1X,A8,'   DATE/CYCLE=',I5,'; SPECIES= ',A,
     >       ' (CODE= ',I3,
     >       '); MINIMUM DBH=',F6.2,'; TOP DIAMETER=',F6.2,
     >       '; STUMP HEIGHT=',F6.2,/T13,'FORM CLASS=',F6.2,
     >       ';  METH OF VOL CALC=',F6.0)
 4547     FORMAT(/1X,A8,'   DATE/CYCLE=',I5,'; SPECIES= ',A,
     >       ' (CODE= ',I3,
     >       '); MINIMUM DBH=',F6.2,'; TOP DIAMETER CAN NOT BE ',
     >       'MODIFIED.'/T13,'IT WILL BE SET TO DEFAULT VALUE',
     >       '; STUMP HEIGHT=',F6.2,' FORM CLASS=',F6.2,
     >       ';  METH OF VOL CALC=',F6.0)
 4548     FORMAT(/1X,A8,'   DATE/CYCLE=',I5,'; SPECIES= ',A,
     >       ' (CODE= ',I3,
     >       '); MINIMUM DBH=',F6.2,'; TOP DIAMETER CAN NOT BE ',
     >       'MODIFIED.'/T13,'IT WILL BE SET TO DEFAULT VALUE BASED ON',
     >       ' NATIONAL FOREST AND SPECIES; STUMP HEIGHT=',F6.2,/T13,
     >       'FORM CLASS=',F6.2,';  METH OF VOL CALC=',F6.0)
...
C----------
      IF(IS .LT. 0) THEN
        ...
        IF (LNOTBK(4)) BFTOPD(IGSP)=ARRAY(4)
        IF(LNOTBK(4).AND.(ITOPD8.LE.0.AND.ITOPD9.LE.0))THEN
           BFTOPD(IGSP)=ARRAY(4)
        ENDIF
        IF (LNOTBK(5)) BFSTMP(IGSP)=ARRAY(5)
        IF (LNOTBK(6)) FRMCLS(IGSP)=ARRAY(6)
        IF (LNOTBK(7)) METHB(IGSP) =ARRAY(7)
        ILEN=ISPGRP(IGRP,52)
        IF (VVER(:2) .EQ. 'AK') THEN
     ...
     &       BFMIND(IGSP)*INtoCM,BFSTMP(IGSP)*FTtoCM,FRMCLS(IGSP)
        ELSEIF(ITOPD9.GT.0)THEN
           WRITE(JOSTND,4538) KEYWRD,KARD(2)(1:ILEN),IGSP,
     &       BFMIND(IGSP)*INtoCM,BFSTMP(IGSP)*FTtoCM,FRMCLS(IGSP)
        ELSE
...
C----------
     ...
     &      BFTOPD(IS)=ARRAY(4)
          ...
          IF (LNOTBK(6)) FRMCLS(IS)=ARRAY(6)
          IF (LNOTBK(7)) METHB(IS)=ARRAY(7)
 4615   CONTINUE
        IF(LKECHO)WRITE(JOSTND,4620) KEYWRD
 4620   FORMAT (/1X,A8,'   THE FOLLOWING MERCHANTABILITY ',
     >        'STANDARDS HAVE BEEN CHANGED FOR ALL SPECIES:')
        IF (LNOTBK(3).AND.LKECHO)WRITE(JOSTND,4621) ARRAY(3)*INtoCM
        IF (LNOTBK(4).AND.(ITOPD8.GT.0))THEN
          WRITE(JOSTND,4527)
        ELSEIF(LNOTBK(4).AND.(ITOPD9.GT.0))THEN
          WRITE(JOSTND,4528)
        ELSEIF(LNOTBK(4).AND.LKECHO)THEN
          WRITE(JOSTND,4522) ARRAY(4)*INtoCM
        ENDIF
        IF (LNOTBK(5).AND.LKECHO)WRITE(JOSTND,4623) ARRAY(5)*FTtoCM
        IF (LNOTBK(6)) THEN
          IF(VVER(:2) .EQ. 'AK') THEN
 ...
 4621   FORMAT (T23,'MINIMUM DBH =',F6.2)
 4622   FORMAT (T23,'TOP DIAMETER=',F6.2)
 4623   FORMAT (T23,'STUMP HEIGHT=',F6.2)
 4624   FORMAT (T23,'FORM CLASS  =',F6.2)
 4625   FORMAT (T23,'METHOD OF VOLUME CALCULATION IS ',F5.0)
 4626   FORMAT (T23,'LOG LENGTH  =',F6.2)
        GOTO 10
      ELSE
C----------
C     SPECIES CODE IS SPECIFIED (IS>0).
C----------
 ...
 4634     FORMAT (/1X,A8,'   MERCHANTABILITY STANDARDS FOR ',
     >      'SPECIES= ',A,' (CODE=',I3,') ARE:  MINIMUM DBH=',F6.2,
     >      '; TOP DIAMETER=',F6.2,'; STUMP HEIGHT=',F6.2/T13,
     >      'LOG LENGTH=',F6.2)
        ELSEIF(LNOTBK(4).AND.(ITOPD8.GT.0))THEN
          IF(LKECHO)WRITE(JOSTND,4537) KEYWRD,KARD(2)(1:ILEN),
     &      IS,BFMIND(IS)*INtoCM,STMP(IS)*FTtoCM,FRMCLS(IS)
        ELSEIF(LNOTBK(4).AND.(ITOPD9.GT.0))THEN
          IF(LKECHO)WRITE(JOSTND,4538) KEYWRD,KARD(2)(1:ILEN),
     &      IS,BFMIND(IS)*INtoCM,STMP(IS)*FTtoCM,FRMCLS(IS)           
        ELSE
     ...
     &      BFSTMP(IS)*FTtoCM,
     &      FRMCLS(IS)
 4635     FORMAT (/1X,A8,'   MERCHANTABILITY STANDARDS FOR ',
     >      'SPECIES= ',A,' (CODE=',I3,') ARE:  MINIMUM DBH=',F6.2,
     >      '; TOP DIAMETER=',F6.2,'; STUMP HEIGHT=',F6.2/T13,
     >      'FORM CLASS=',F6.2)
        ENDIF
...
C----------
 ...
 4674     FORMAT(/1X,A8,'   DATE/CYCLE=',I5,'; ALL SPECIES (CODE= 0)',
     >      '; MINIMUM DBH=',F6.2,'; TOP DIAMETER=',F6.2,
     >      '; STUMP HEIGHT=',F6.2/T13,'LOG LENGTH=',F6.2,
     >      '; METH OF VOL CALC=',F6.0)
        ELSEIF(ITOPD8.GT.0)THEN
     ...
     >     (ARRAY(I),I=6,7)
        ELSE
          IF(LKECHO)WRITE(JOSTND,4675) KEYWRD,IDT,
     >      ARRAY(3)*INtoCM,
     >      ARRAY(4)*INtoCM,
     >      ARRAY(5)*FTtoCM,
     >      ARRAY(6),ARRAY(7)
 4675     FORMAT(/1X,A8,'   DATE/CYCLE=',I5,'; ALL SPECIES (CODE= 0)',
     >      '; MINIMUM DBH=',F6.2,'; TOP DIAMETER=',F6.2,
     >      '; STUMP HEIGHT=',F6.2/T13,'FORM CLASS=',F6.2,
     >      '; METH OF VOL CALC=',F6.0)
        ENDIF 
...
C----------
        ...
        IF(VVER(:2).EQ.'AK')THEN
          IF(LKECHO)WRITE(JOSTND,4645) KEYWRD,IDT,KARD(2)(1:ILEN),IS,
     >      (ARRAY(I),I=3,7)
 4645     FORMAT(/1X,A8,'   DATE/CYCLE=',I5,'; SPECIES= ',A,
     >      ' (CODE= ',I3,
     >      '); MINIMUM DBH=',F6.2,'; TOP DIAMETER=',F6.2,
     >      '; STUMP HEIGHT=',F6.2,/T13,'LOG LENGTH=',F6.2,
     >      '; METH OF VOL CALC=',F6.0)
     ...
     >      ARRAY(5)*FTtoCM,
     >      ARRAY(6),ARRAY(7)
 4646     FORMAT(/1X,A8,'   DATE/CYCLE=',I5,'; SPECIES= ',A,
     >      ' (CODE= ',I3,
     >      '); MINIMUM DBH=',F6.2,'; TOP DIAMETER=',F6.2,
     >      '; STUMP HEIGHT=',F6.2,/T13,'FORM CLASS=',F6.2,
     >      '; METH OF VOL CALC=',F6.0)
...
C
 5000 CONTINUE
      ARRAY(2)=ARRAY(2)*M3pHAtoFT3pACR
      ARRAY(3)=0.0 ! bd feet not used
      ARRAY(4)=ARRAY(4)*M2pHAtoFT2pACR
      ARRAY(5)=ARRAY(5)*M3pHAtoFT3pACR
      IDT=1
      IF (LNOTBK(1)) IDT=IFIX(ARRAY(1))
C
C     IF THE KEYWORD RECORD HAS THE 'PARMS' OPTION, CALL OPNEWC
...
C
     ...
     &    VVER(:2).EQ.'SN' .OR. VVER(:2).EQ.'ON') THEN
        IF(LKECHO)WRITE(JOSTND,5010) KEYWRD,IDT,
     >    ARRAY(2)*FT3pACRtoM3pHA,
     >    ARRAY(3),
     >    ARRAY(4)*FT2pACRtoM2pHA,
     >    ARRAY(5)*FT3pACRtoM3pHA
 5010   FORMAT (/1X,A8,'   DATE/CYCLE=',I5,'; CUTTING MUST EXCEED ',
     >    'THE FOLLOWING:',/,30X,F8.1,' GROSS MERCHANTABLE M3/HA,',
     >    F8.1,' SAWLOG BDFT,',F6.1,' M2/HA BASAL AREA, AND',
     >    F8.1,' TOTAL MERCH M3/HA.')
      ELSE
        IF(LKECHO)WRITE(JOSTND,5010) KEYWRD,IDT,
     >    ARRAY(2)*FT3pACRtoM3pHA,
     >    ARRAY(3),
     >    ARRAY(4)*FT2pACRtoM2pHA,
     >    ARRAY(5)*FT3pACRtoM3pHA
 5011   FORMAT (/1X,A8,'   DATE/CYCLE=',I5,'; CUTTING MUST EXCEED ',
     >    'THE FOLLOWING:',/,30X,F8.1,' MERCH M3/HA,',F8.1,
     >    ' MERCH BDFT,',F6.1,' M2/HA BASAL AREA, AND',
     >    F8.1,' TOTAL M3/HA.')
      ENDIF
      GO TO 10
C
...
C
     ...
     >  ARRAY(3)
 5160 FORMAT (/1X,A8,'   DATE/CYCLE=',I5,'; SPECIES= ',A,' (CODE=',
...
C
 ...
 5410 FORMAT (/1X,A8,'   PROPORTION OF SELECTED TREES REMOVED=',
...
C
 ...
 5610 FORMAT (/1X,A8,'   LARGE TREE DIAMETER GROWTH MODELS ',
 ...
 5660 FORMAT (T13,'LARGE TREE DIAMETER GROWTH MODEL ',
     >        'CORRECTION TERMS BY SPECIES:')
      IF(LKECHO)WRITE(JOSTND,5670) (NSP(I,1),COR2(I),I=1,MAXSP)
 5670 FORMAT((T13,7(A2,' =',F5.2,:,'; '),A2,' =',F5.2))
      GO TO 10
C
...
C----------
 ...
 5804   FORMAT(/1X,A8,'   CALIBRATION USING GROWTH DATA FROM TREE',
...
C----------
      ELSE
        ...
        IF(IS.LT.0)ILEN=ISPGRP(-IS,52)
        IF(LKECHO)WRITE(JOSTND,5808) KEYWRD,KARD(1)(1:ILEN),IS
 5808   FORMAT (/1X,A8,'   CALIBRATION USING GROWTH DATA FROM TREE',
...
C
 ...
 5910 FORMAT (/1X,A8,'   RANDOM VARIATION THAT IS ADDED TO PREDICTED ',
     > 'DIAMETER GROWTH IS BOUNDED TO ',F4.1,' STANDARD DEVIATIONS.')
      IF((DGSD.LT.1.0).AND.LKECHO)WRITE(JOSTND,5920)
 5920 FORMAT (T13,'NO RANDOM VARIATION IS ADDED TO DIAMETER ',
     >       'GROWTH PREDICTIONS.')
      GO TO 10
C
...
C
     ...
     >  ARRAY(3)
 6030 FORMAT (/1X,A8,'   DATE/CYCLE=',I5,'; SPECIES=',A,' (CODE= ',
     >        I3,'); MULTIPLIER=',F10.4)
      GOTO 10
C
...
C
 ...
 6131   FORMAT(/1X,A8,'   DATE/CYCLE=',I5,'; SPECIES= ',A,
     ...
     >    ARRAY(4)*INtoCM,
     >    ARRAY(5)*INtoCM
 6130   FORMAT (T13,'ONLY TREES GREATER THAN ',F7.2,
     >    ' AND LESS THAN ',F7.2,' CMS DBH ARE AFFECTED.')
      ENDIF
      GOTO 10
C
...
C
 ...
 6206      FORMAT(/1X,A8,'   CALIBRATION OF HEIGHT-DIAMETER',
 ...
 6220      FORMAT(/1X,A8,'   CALIBRATION OF HEIGHT-DIAMETER',
...
C
         ...
         IF(LNOTBK(2).AND.ARRAY(2).GT.0.) THEN
           LHTDRG(IS)=.TRUE.
           IF(LKECHO)WRITE(JOSTND,6215) KEYWRD,KARD(1)(1:ILEN),IS
 6215      FORMAT (/1X,A8,'   CALIBRATION OF HEIGHT-DIAMETER ',
     >       'RELATIONSHIP WILL BE  INVOKED   FOR SPECIES= ',A,
     >       ' (CODE= ',I3,')')
         ELSE
           LHTDRG(IS)=.FALSE.
           IF(LKECHO)WRITE(JOSTND,6230) KEYWRD,KARD(1)(1:ILEN),IS
 6230      FORMAT (/1X,A8,'   CALIBRATION OF HEIGHT-DIAMETER ',
...
C
 ...
 6310 FORMAT (/1X,A8,'   RANDOM SEED IS:',F14.1)
      GO TO 10
C
...
C
 ...
 6510 FORMAT (T13,'ECONOMIC SUMMARY DEBUG REQUESTED.  LECBUG=',L2)
      CALL ECAVAL
      GO TO 10
C
...
C
 ...
 6610 FORMAT (/1X,A8,'   NUMBER OF TRIPLES LIMITED TO',I3)
      GO TO 10
C
...
C
 6800 CONTINUE
      ...
      IF(ARRAY(1).GT.0.0) THEN
        ...
        LBAMAX=.TRUE.
      ENDIF
      IF(LKECHO)WRITE(JOSTND,6810) KEYWRD,BAMAX*FT2pACRtoM2pHA
 6810 FORMAT (/1X,A8,'   MAXIMUM BASAL AREA=',F10.2)
      GO TO 10
C
...
C
 ...
 6910 FORMAT (/1X,A8,'   LARGE TREE HEIGHT GROWTH MODELS ',
     >  'ARE MODIFIED WITH CORRECTION TERMS PRIOR TO CALIBRATION.')
 ...
 6920 FORMAT (T13,'LARGE TREE HEIGHT GROWTH MODEL ',
     >  'CORRECTION TERMS BY SPECIES:')
...
C
 ...
 7110 FORMAT (T13,'MANAGEMENT ID= ',A4)
      GO TO 10
C
...
C
 ...
 7330 FORMAT (/1X,A8,'   DATE/CYCLE=',I5,
     >  '; TREE SUBCLASS MULTIPLIER=',F6.1,
     >  '; SPECIAL TREE STATUS MULTIPLIER=',F8.1,
     >  ';',/,T13,'POINT BASAL AREA MULTIPLIER=',F8.1,
     >  '; POINT CCF MULTIPLIER=',F8.1,
     >  '; POINT TPH MULTIPLIER=',F8.1)
      GOTO 10
C
...
C
 ...
 7510 FORMAT (/1X,A8,'   SMALL TREE HEIGHT GROWTH MODELS ',
     >  'ARE MODIFIED WITH CORRECTION TERMS PRIOR TO CALIBRATION.')
 ...
 7520 FORMAT (T13,'SMALL TREE HEIGHT GROWTH MODEL ',
     >  'CORRECTION TERMS BY SPECIES:')
...
C
 ...
 7710 FORMAT(/,1X,A8,'   BLISTER RUST KEYWORDS:')
...
C
 7900 CONTINUE
      LSCRN=.TRUE.
      IF (LNOTBK(1)) JOSCRN=ARRAY(1)
      IF(LKECHO)WRITE(JOSTND,7910) KEYWRD,JOSCRN
 7910 FORMAT (/1X,A8,'   SUMMARY TABLE WILL BE PRINTED TO DATA SET ',
...
C
 ...
 8020 FORMAT(/1X,A8,'   DATE/CYCLE=',I5,'; TARGET TREE RECORDS=',F5.0,
     >  '; FIND ',F5.0,' PERCENT OF THE NEW RECORDS BY DEFINING'/
     >  T13,'TREE-CLASS BOUNDARIES WHERE THE LARGEST ',
     >  'DIFFERENCES ARE BETWEEN TREES.  DEFINE THE REMAINING'/
     >  T13,'CLASSES BY SPLITTING THOSE WHICH HAVE THE LARGEST ',
     >  'RANGES.')
      GO TO 10
C
...
C
 ...
 8410 FORMAT (/1X,A8,'   NO PROJECTABLE TREE RECORDS EXPECTED.')
      CALL ESEZCR(JOSTND,LKECHO)
      GOTO 10
C
...
C
 ...
 8510 FORMAT (/1X,A8,'   CALIBRATION STATISTICS WILL BE ',
     >        'WRITTEN TO FILE REFERENCED BY NUMBER ',I2/
     >        T13,'MINIMUM NUMBER OF DIA. GROWTH OBSERVATIONS PER ',
     >        'SPECIES TO CALIBRATE '/
     >        T13,'THE LARGE TREE DIAMETER INCREMENT ',
...
C
 ...
 8720 FORMAT (/1X,A8,'   DATA SET REFERENCE NUMBER = ',I5)
      CLOSE (UNIT=I)
      GOTO 10
C
...
C
 ...
 8810 FORMAT (/1X,A8,'   SCREEN OUTPUT DEACTIVATED.')
      GOTO 10
C
...
C
 ...
 8910 FORMAT (/,1X,A8,'   ROOT DISEASE MODEL IS OBSOLETE, USE',/,
...
C
     ...
     &                ARRAY,KARD)
         ...
         ILEN=3
         IF(IS.LT.0)ILEN=ISPGRP(-IS,52)
         IF(LKECHO)WRITE(JOSTND,9030) KEYWRD,IDT,KARD(2)(1:ILEN),IS,
     >     ARRAY(3),
     >     ARRAY(4)*INtoCM,
     >     ARRAY(5)*INtoCM,
     >     ARRAY(6)
 9030    FORMAT (/1X,A8,'   DATE/CYCLE=',I5,'; SPECIES=',A,' (CODE=',
     >    I3,'); MORTALITY RATE=',F10.4/
     >    T13,'ONLY TREES GREATER THAN ',F7.2,' AND LESS THAN ',
     >    F7.2,' CM DBH ARE AFFECTED.'/T13,'TYPE CODE = ',
...
C----------
        ELSEIF(IS .EQ. 0)THEN
          ...
          MAXSDI(J)=1          
 9103     CONTINUE
          IF(LKECHO)WRITE(JOSTND,9104) ARRAY(2)/ACRtoHA
 9104     FORMAT(/9X,'   MAXIMUM PERMISSABLE STAND DENSITY INDEX',
...
C----------
        ELSE
          ...
          MAXSDI(IS)=1
          ILEN=3
          IF(LKECHO)WRITE(JOSTND,9108) KARD(1)(1:ILEN),IS,
     >      ARRAY(2)/ACRtoHA
 9108     FORMAT (/9X,'   MAXIMUM PERMISSABLE STAND DENSITY INDEX',
...
C
      IF(VVER(:2).NE.'SM' .AND. VVER(:2).NE.'SP' .AND.
     &VVER(:2).NE.'BP' .AND. VVER(:2).NE.'LP' .AND.
     &VVER(:2).NE.'SF') GO TO 9120
C
      IF(LNOTBK(7)) THEN
        IF(IS .LT. 0) THEN
          IGRP = -IS
 ...
 9116     FORMAT(/9X,'   STAGNATION EFFECTS INDICATOR',
...
C
        ...
        ILEN=3
        IF(LKECHO)WRITE(JOSTND,9117) KARD(1)(1:ILEN),IS,ARRAY(7)
 9117   FORMAT (/9X,'   STAGNATION EFFECTS INDICATOR',
     >    ' WILL BE CHANGED FOR SPECIES= ',A,' (CODE= ',I3,
     >    '); INDICATOR =',F4.0)
        ENDIF
      ENDIF
...
C
 ...
 9215   FORMAT(/1X,A8,'   NO TABLE SPECIFIED, ALL OPTIONAL TABLES',
     &  ' WILL BE DELETED.')
        GO TO 10
      ENDIF
C
 ...
 9220   FORMAT(/1X,A8,'   TABLE NUMBER',I4,'  SPECIFIED. THIS IS',
...
C
 ...
 9310 FORMAT (/1X,A8,'   ARMA(1,1) SERIAL CORRELATION PARAMETERS:',
     >      ' PHI=',F5.2,'; THETA=',F5.2)
      GOTO 10
C
...
C
 ...
 9410 FORMAT (/1X,A8,'   DATE/CYCLE=',I5,'; DATA SET REFERENCE ',
     >        'NUMBER =',F4.0,'; HEADING SUPPRESSION CODE =',F3.0/
     >        T13,'(0=WITH HEADING, OTHER VALUES=SUPPRESS HEADING).')
      IF((NP.EQ.3 .AND. ARRAY(4).EQ.2).AND.LKECHO)WRITE(JOSTND,9411)
 9411 FORMAT(T13,'CYCLE ONE CUTLIST SUPRESSED')
      IF((NP.EQ.5 .AND. ARRAY(6).GT.0).AND.LKECHO)WRITE(JOSTND,9412)
 9412 FORMAT(T13,'CUTLIST WILL BE IN OLD FORMAT')
      GOTO 10
C
...
C
 ...
 9510 FORMAT (/1X,A8,'   DATE/CYCLE=',I5,'; NEW AGE=',F6.0)
      GOTO 10
C
...
C----------
 ...
 9605   CONTINUE
        ILEN=ISPGRP(-IS,52)
        IF(LKECHO)WRITE(JOSTND,9610) KEYWRD,KARD(1)(1:ILEN),IS,
     >    ARRAY(2)*FTtoM
C----------
C     IF (IS=0) ALL SPECIES WILL BE CHANGED IN FOLLOWING CODE:
C----------
 ...
 9616     CONTINUE
 9617     CONTINUE
          IF(LKECHO)WRITE(JOSTND,9618) KEYWRD,ARRAY(2)*FTtoM
 9618     FORMAT(/1X,A8,'   SITE CODE CHANGED FOR ALL SPECIES; ',
...
C
        ILEN=3
        IF(LKECHO)WRITE(JOSTND,9610) KEYWRD,KARD(1)(1:3),IS,
     >    ARRAY(2)*FTtoM
 9610   FORMAT(/1X,A8,'   SITE INDEX SPECIES= ',A,' (CODE= ',I3,
     >    ');  SITE CODE FOR LARGE AND SMALL TREES=',F8.1)
C
...
C
 ...
 9611     FORMAT(9X,'   SITE INDEX SPECIES WILL BE USED AS THE SITE',
     &         ' SPECIES.')
        ENDIF
      ENDIF
C
...
C
 ...
 9620     FORMAT(9X,'   FIELD 2 OF THE SITECODE KEWORD IS LESS',
...
C
     ...
     &             ARRAY,KARD)
      ...
      IF (KODE.GT.0) GOTO 10
      IF (ARRAY(5) .LE. 0.0) ARRAY(5)=99.0*CMtoIN
      ILEN=3
      IF(IS.LT.0)ILEN=ISPGRP(-IS,52)
      IF(LKECHO)WRITE(JOSTND,9854)KEYWRD,IDT,KARD(2)(1:ILEN),IS,
     >  ARRAY(3),
     &  ARRAY(4)*INtoCM,
     &  ARRAY(5)*INtoCM, ARRAY(6)
 9854 FORMAT(/1X,A8,'   DATE/CYCLE=',I5,';  SPECIES=',A,' (CODE= ',I3,
...
C----------
 ...
 9908   FORMAT(/1X,A8,T13,'DEFAULT COEFFICIENTS FOR CUBIC METRE VOLUME'/
     &              T13,'EQUATIONS REPLACED FOR ALL SPECIES.')
C----------
...
C----------
 ...
 9918   FORMAT(/1X,A8,T13,'DEFAULT COEFFICIENTS FOR CUBIC METRE VOLUME'/
     &    T13,'EQUATIONS REPLACED FOR SPECIES= ',A,' (CODE= ',I3,').' )
      ENDIF
C
 9920 CONTINUE
      IF(LKECHO)WRITE(JOSTND,9925) ITC,TC,(WK6(I),I=1,14)
 9925 FORMAT(T13,'TRANSITION SIZE CODE = ',I4,';   TRANSITION SIZE = ',
     &  F10.2/T13,'COEFFICIENTS FOR TREE THAT IS SMALLER THAN',
     &  ' TRANSITION SIZE:'/T13,7F12.5/
     &        T13,'COEFFICIENTS FOR TREE THAT IS LARGER THAN',
     &  ' TRANSITION SIZE:'/T13,7F12.5)
      GO TO 10
C
...
C----------
 ...
 9958   FORMAT(/1X,A8,T13,'DEFAULT COEFFICIENTS FOR BOARD ',
     &    'METRIC VOLUME'/
     &    T13,'EQUATIONS REPLACED FOR ALL SPECIES.')
C----------
C     SPECIES CODE IS SPECIFIED (ISPC>0).
C----------
 ...
 9965   CONTINUE
        ILEN=3
        IF(LKECHO)WRITE(JOSTND,9968) KEYWRD,KARD(1)(1:ILEN),ISPC
 9968   FORMAT(/1X,A8,T13,'DEFAULT COEFFICIENTS FOR BOARD ',
     &   'METRIC VOLUME'/
     &    T13,'EQUATIONS REPLACED FOR SPECIES= ',A,' (CODE= ',I3,').' )
      ENDIF
C
 ...
 9972 FORMAT(T13,'TRANSITION SIZE CODE = ',I4,';   TRANSITION SIZE = ',
     &  F10.2/T13,'COEFFICIENTS FOR TREE THAT IS SMALLER THAN',
     &  ' TRANSITION SIZE:'/T13,7F12.5/
     &        T13,'COEFFICIENTS FOR TREE THAT IS LARGER THAN',
     &  ' TRANSITION SIZE:'/T13,7F12.5)
...
C
 ...
 9977 FORMAT (/,1X,A8,'   ANNOSUS ROOT DISEASE MODEL IS OBSOLETE, USE',/
     &                ' WESTERN ROOT DISEASE MODEL VER. 3.0:')
      GOTO 10
C
...
C
 ...
 9987 FORMAT (/,1X,A8,'   DOUGLAS-FIR BEETLE OPTIONS:')
...
C
 ...
 9997 FORMAT (/,1X,A8,'   WESTERN ROOT DISEASE MODEL OPTIONS:')
...
10010   FORMAT(/1X,A8,'   DATE/CYCLE=',I5,'; MANAGED = ',F5.0)
      ELSE
        IF(LNOTBK(2) .AND. ARRAY(2).EQ.0.) THEN
          IF(LKECHO)WRITE(JOSTND,10015) KEYWRD,IDT
10015     FORMAT(/1X,A8,'   DATE/CYCLE=',I5,'; MANAGED = 0 (NO)')
        ELSE
          IF(LKECHO)WRITE(JOSTND,10020) KEYWRD,IDT
10020     FORMAT(/1X,A8,'   DATE/CYCLE=',I5,'; MANAGED = 1 (YES)')
          ARRAY(2)=1.
        ENDIF
      ENDIF
...
C
     ...
     >  ARRAY(4)
10110 FORMAT (/1X,A8,'   DATE/CYCLE=',I5,
     >  '; PROPORTION OF HARVESTED STEMS LEFT IN STAND = ',F6.4/T13,
     >  'PROPORTION OF NON-REMOVED HARVEST THAT IS DOWN = ',F6.4/T13,
...
C
C  FOR SE VARIANT, DECODE PHYSIOGRAPHIC REGION. VALUE GETS STORED
C  IN VARIABLE IPHREG, WITH ALPHA REPRESENTATION IN VARIABLE PCOM.
C----------
      IF(VVER(:2).EQ.'SE') THEN
        IF(LNOTBK(1)) IMODTY=IFIX(ARRAY(1))
        IF(LNOTBK(3)) IFORSE=IFIX(ARRAY(3))
        IF(LKECHO)WRITE(JOSTND,10410) KEYWRD,IMODTY,KARD(2),IFORSE
10410   FORMAT(/1X,A8,'   MODEL TYPE =',I5,'; PHYSIOGRAPHIC REGION =',
     &          A10,'; FOREST TYPE =',I5)
        IF(LKECHO)WRITE(JOSTND,10420)
10420   FORMAT(1X,T13,'(SEE MODTYPE KEYWORD DISCUSSION FOLLOWING',
     &  ' --- PROCESS --- KEYWORD.)')
        CALL HABTYP (KARD(2),ARRAY(2))
C----------
C  FOR CENTRAL ROCKIES VARIANT, ONLY DEAL WITH MODEL TYPE
C  OTHER PARAMETER HAVE NO EFFECT.
C----------
...
10430   FORMAT(/1X,A8,'   MODEL TYPE =',I5)
        IF(LNOTBK(2) .OR. LNOTBK(3)) THEN
          IF(LKECHO)WRITE(JOSTND,10440)
10440     FORMAT(1X,T13,'OTHER PARAMETERS IGNORED')
        ENDIF
        IF(LKECHO)WRITE(JOSTND,10450)
10450   FORMAT(1X,T13,'(SEE MODTYPE KEYWORD DISCUSSION FOLLOWING',
     &  ' --- PROCESS --- KEYWORD.)')
C
      ELSE
...
C
        ...
        IF(LKECHO)WRITE(JOSTND,10440)
10455   FORMAT(/1X,A8,T13,'FOREST TYPE =',I5)
      ENDIF
      GO TO 10
C
...
10510   FORMAT (/1X,A8,'   DATE/CYCLE=',I5,
...
C
     ...
     &           ARRAY,KARD)
      ...
      ILEN=3
      IF(IS.LT.0)ILEN=ISPGRP(-IS,52)
      IF(LKECHO)WRITE(JOSTND,10606)KEYWRD,IDT,(ARRAY(J),J=2,4),
     &    KARD(5)(1:ILEN),IS,(ARRAY(J),J=6,7)
10606 FORMAT(/1X,A8,'   DATE/CYCLE=',I5,';  METHOD=',F5.0,
     &    ';  M=',F7.2,';  MAX CR REMOVAL PROPORTION=',F6.1,
     &    ';'/T13,'SPECIES=',A,' (CODE= ',I3,
...
C
     ...
     &                ARRAY,KARD)
         ...
         ILEN=3
         IF(IS.LT.0)ILEN=ISPGRP(-IS,52)
         IF(LKECHO)WRITE(JOSTND,11030) KEYWRD,IDT,KARD(2)(1:ILEN),IS,
     >     ARRAY(3),
     >     ARRAY(4)*INtoCM,
     >     ARRAY(5)*INtoCM
11030    FORMAT (/1X,A8,'   DATE/CYCLE=',I5,'; SPECIES=',A,' (CODE=',
...
C
     ...
     &                ARRAY,KARD)
         ...
         ILEN=3
         IF(IS.LT.0)ILEN=ISPGRP(-IS,52)
         IF(LKECHO)WRITE(JOSTND,11130) KEYWRD,IDT,KARD(2)(1:ILEN),IS,
     >     ARRAY(3),
     >     ARRAY(4)*INtoCM,
     >     ARRAY(5)*INtoCM
11130    FORMAT (/1X,A8,'   DATE/CYCLE=',I5,'; SPECIES=',A,' (CODE=',
...
C
     ...
     &             ARRAY,KARD)
      ...
      IF(PRMS(5) .LE. PRMS(4))THEN
         ...
         GOTO 10
      ENDIF
	PRMS(4)= PRMS(4) * CMtoIN
	PRMS(5)= PRMS(5) * CMtoIN
      PRMS(6)=0.
      IF(LNOTBK(7)) PRMS(6)=ARRAY(7)
...
11220 FORMAT (/1X,A8,'   DATE/CYCLE=',I5,'; RESIDUAL=',F8.2)
      IF(PRMS(2).EQ.0.)THEN
        ILEN=3
        IF(IS.LT.0)ILEN=ISPGRP(-IS,52)
        IF(LKECHO)WRITE(JOSTND,11221)KARD(4)(1:ILEN),IS,
     &    PRMS(4)*INtoCM,
     >    PRMS(5)*INtoCM
11221   FORMAT(1X,T13,
     ...
     >   T13,'SPECIES=',A,' (CODE=',I3,');',
     >   ' CM OF REMOVED TREES WILL RANGE FROM ',F5.1,' TO ',
     >   F5.1,' CM.')
      ELSE
        ILEN=3
        IF(IS.LT.0)ILEN=ISPGRP(-IS,52)
        IF(LKECHO)WRITE(JOSTND,11222)PRMS(2),KARD(4)(1:ILEN),IS,
     &    PRMS(4)*INtoCM,
     >    PRMS(5)*INtoCM
11222   FORMAT(1X,T13,
     >   'PROPORTION OF SELECTED TREES REMOVED= ',F6.3,';',/,
...
11223   FORMAT(1X,T13,'CUTTING WILL BE DISTRIBUTED THROUGHOUT THE ',
     >                'SPECIFIED DIAMETER CLASS.')
      ELSEIF (PRMS(6) .EQ. 1) THEN
        IF(LKECHO)WRITE(JOSTND,11224)
11224   FORMAT(1X,T13,'CUTTING WILL BE FROM BELOW IN THE SPECIFIED ',
     >                'DIAMETER CLASS.')
      ELSE
        IF(LKECHO)WRITE(JOSTND,11225)
11225   FORMAT(1X,T13,'CUTTING WILL BE FROM ABOVE IN THE SPECIFIED ',
     >                'DIAMETER CLASS.')
      ENDIF
      GOTO 10
C
...
11310 FORMAT (/1X,A8,'   LATITUDE =',F9.4,';  LONGITUDE = ',F9.4,
...
11410 FORMAT (/,1X,A8,'   BGC KEYWORDS:')
      CALL BGCIN (KEYWRD,ARRAY,LNOTBK,LKECHO)
      GOTO 10
C
...
C
     ...
     &             ARRAY,KARD)
      ...
      PRMS(3)=IS
      PRMS(4)=0.0
      IF(LNOTBK(5)) PRMS(4)=ARRAY(5)*CMtoIN
      PRMS(5)=999.0*CMtoIN
      IF(LNOTBK(6)) PRMS(5)=ARRAY(6)*CMtoIN
      IF(PRMS(5) .LE. PRMS(4))THEN
         CALL KEYDMP (JOSTND,IRECNT,KEYWRD,ARRAY,KARD)
         CALL ERRGRO (.TRUE.,4)
         GOTO 10
...
11520 FORMAT (/1X,A8,'   DATE/CYCLE=',I5,'; RESIDUAL=',F8.2)
      IF(PRMS(2).EQ.0.)THEN
        ILEN=3
        IF(IS.LT.0)ILEN=ISPGRP(-IS,52)
        IF(LKECHO)WRITE(JOSTND,11521)KARD(4)(1:ILEN),IS,
     >    PRMS(4)*INtoCM,
     >    PRMS(5)*INtoCM
11521   FORMAT(1X,T13,
     >   'PROPORTION OF SELECTED TREES REMOVED WILL BE ',
     ...
     >   F5.1,' CM.')
      ELSE
        ILEN=3
        IF(IS.LT.0)ILEN=ISPGRP(-IS,52)
        IF(LKECHO)WRITE(JOSTND,11522)PRMS(2),KARD(4)(1:ILEN),IS,
     >    PRMS(4)*INtoCM,
     >    PRMS(5)*INtoCM
11522   FORMAT(1X,T13,
     >   'PROPORTION OF SELECTED TREES REMOVED= ',F6.3,';',/,
...
11523   FORMAT(1X,T13,'CUTTING WILL BE DISTRIBUTED THROUGHOUT THE ',
     >                'SPECIFIED DIAMETER CLASS.')
      ELSEIF (PRMS(6) .EQ. 1) THEN
        IF(LKECHO)WRITE(JOSTND,11524)
11524   FORMAT(1X,T13,'CUTTING WILL BE FROM BELOW IN THE SPECIFIED ',
     >                'DIAMETER CLASS.')
      ELSE
        IF(LKECHO)WRITE(JOSTND,11525)
11525   FORMAT(1X,T13,'CUTTING WILL BE FROM ABOVE IN THE SPECIFIED ',
     >                'DIAMETER CLASS.')
      ENDIF
      GOTO 10
C
...
11810    FORMAT (/,1X,A8,'   SYSTEM CALL:',A)
         CALL SYSTEM(RECORD)
      ELSE
         IF (IPRMPT.GT.0) THEN
            IF (IPRMPT.NE.2) THEN
...
11820    FORMAT (/,1X,A8,'   DATE/CYCLE=',I5,'; SYSTEM CALL:',A)
...
11901   CONTINUE
     ...
     &   VVER(:2).EQ.'ON') THEN
          IF(IDTYPE.EQ.0 .OR. IDTYPE.EQ.1)THEN
            IF(LKECHO)WRITE(JOSTND,11932) KEYWRD,KARD(2)(1:ILEN),IS,
     >        (WK6(I),I=1,8)
          ENDIF
          IF(IDTYPE.EQ.0 .OR. IDTYPE.EQ.2)THEN
            IF(LKECHO)WRITE(JOSTND,11933) KEYWRD,KARD(2)(1:ILEN),IS,
     >        (WK6(I),I=1,8)
          ENDIF
        ELSE
          IF(IDTYPE.EQ.0 .OR. IDTYPE.EQ.1)THEN
            IF(LKECHO)WRITE(JOSTND,11934) KEYWRD,KARD(2)(1:ILEN),IS,
     >        (WK6(I),I=1,8)
          ENDIF
          IF(IDTYPE.EQ.0 .OR. IDTYPE.EQ.2)THEN
            IF(LKECHO)WRITE(JOSTND,11935) KEYWRD,KARD(2)(1:ILEN),IS,
     >        (WK6(I),I=1,8)
          ENDIF
        ENDIF
C----------
...
11912       FORMAT(/1X,A8,T13,'GROSS TOTAL VOLUME DEFECT',
     >       ' PROPORTIONS HAVE BEEN CHANGED FOR ALL SPECIES:',
     >       /T13,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2,
     >       '; 38.1 CM TREES=',F6.2,/T13,'50.8 CM TREES=',F6.2,
     >       '; 63.5 CM TREES=',F6.2,'; 76.2 CM TREES=',F6.2,/T13,
     >       '88.9 CM TREES=',F6.2,'; 101.6 CM AND LARGER TREES=',F6.2)
            IF(LKECHO .AND. IGNDEF.GT.0)WRITE(JOSTND,11909)
11909       FORMAT(/1X,T13,'DEFECT PERCENTAGES READ AS PART OF THE TRE',
     >      'E RECORD INPUT FOR ALL SPECIES WILL BE IGNORED.')
          ENDIF
          IF(IDTYPE.EQ.0 .OR. IDTYPE.EQ.2)THEN
            IF(LKECHO)WRITE(JOSTND,11913) KEYWRD,(WK6(I),I=1,8)
11913       FORMAT(/1X,A8,T13,'GROSS MERCHANTABLE VOLUME DEFECT',
     &      ' PROPORTIONS HAVE BEEN CHANGED FOR ALL SPECIES:',
     >       /T13,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2,
     >       '; 38.1 CM TREES=',F6.2,/T13,'50.8 CM TREES=',F6.2,
     >       '; 63.5 CM TREES=',F6.2,'; 76.2 CM TREES=',F6.2,/T13,
...
11920       FORMAT(/1X,A8,T13,'CUBIC FOOT VOLUME DEFECT',
     >       ' PROPORTIONS HAVE BEEN CHANGED FOR ALL SPECIES:',
     >       /T13,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2,
     >       '; 38.1 CM TREES=',F6.2,/T13,'50.8 CM TREES=',F6.2,
     >       '; 63.5 CM TREES=',F6.2,'; 76.2 CM TREES=',F6.2,/T13,
...
11921       FORMAT(/1X,A8,T13,'BOARD FOOT VOLUME DEFECT',
     &      ' PROPORTIONS HAVE BEEN CHANGED FOR ALL SPECIES:',
     >       /T13,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2,
     >       '; 38.1 CM TREES=',F6.2,/T13,'50.8 CM TREES=',F6.2,
     >       '; 63.5 CM TREES=',F6.2,'; 76.2 CM TREES=',F6.2,/T13,
...
C----------
     ...
     &   VVER(:2).EQ.'ON') THEN
          IF(IDTYPE.EQ.0 .OR. IDTYPE.EQ.1)THEN
            IF(LKECHO)WRITE(JOSTND,11932) KEYWRD,KARD(2)(1:ILEN),IS,
     >        (WK6(I),I=1,8)
11932       FORMAT(/1X,A8,T13,'GROSS TOTAL VOLUME DEFECT PROPORTIONS ',
     >        'HAVE BEEN CHANGED FOR SPECIES:',A,' (CODE=',I3,')',
     >        /T13,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2,
     >        '; 38.1 CM TREES=',F6.2,/T13,'50.8 CM TREES=',F6.2,
     >        '; 63.5 CM TREES=',F6.2,'; 76.2 CM TREES=',F6.2,/T13,
     >        '88.9 CM TREES=',F6.2,'; 101.6 CM AND LARGER TREES=',F6.2)
          ...
          IF(IDTYPE.EQ.0 .OR. IDTYPE.EQ.2)THEN
            IF(LKECHO)WRITE(JOSTND,11933) KEYWRD,KARD(2)(1:ILEN),IS,
     >        (WK6(I),I=1,8)
11933       FORMAT(/1X,A8,T13,'GROSS MERCHANTABLE VOLUME DEFECT ',
     &        'PROPORTIONS HAVE BEEN CHANGED FOR SPECIES:',A,
     &        ' (CODE=',I3,')',
     >        /T13,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2,
     >        '; 38.1 CM TREES=',F6.2,/T13,'50.8 CM TREES=',F6.2,
     ...
     >        '88.9 CM TREES=',F6.2,'; 101.6 CM AND LARGER TREES=',F6.2)
        ...
        ELSE
          IF(IDTYPE.EQ.0 .OR. IDTYPE.EQ.1)THEN
            IF(LKECHO)WRITE(JOSTND,11934) KEYWRD,KARD(2)(1:ILEN),IS,
     >        (WK6(I),I=1,8)
11934       FORMAT(/1X,A8,T13,'CUBIC FOOT VOLUME DEFECT PROPORTIONS ',
     >       'HAVE BEEN CHANGED FOR SPECIES:',A,' (CODE=',I3,')',
     >       /T13,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2,
     >       '; 38.1 CM TREES=',F6.2,/T13,'50.8 CM TREES=',F6.2,
     >       '; 63.5 CM TREES=',F6.2,'; 76.2 CM TREES=',F6.2,/T13,
     >       '88.9 CM TREES=',F6.2,'; 101.6 CM AND LARGER TREES=',F6.2)
          ...
          IF(IDTYPE.EQ.0 .OR. IDTYPE.EQ.2)THEN
            IF(LKECHO)WRITE(JOSTND,11935) KEYWRD,KARD(2)(1:ILEN),IS,
     >        (WK6(I),I=1,8)
11935       FORMAT(/1X,A8,T13,'BOARD FOOT VOLUME DEFECT PROPORTIONS ',
     &      'HAVE BEEN CHANGED FOR SPECIES:',A,' (CODE=',I3,')',
     >       /T13,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2,
     >       '; 38.1 CM TREES=',F6.2,/T13,'50.8 CM TREES=',F6.2,
     >       '; 63.5 CM TREES=',F6.2,'; 76.2 CM TREES=',F6.2,/T13,
     >       '88.9 CM TREES=',F6.2,'; 101.6 CM AND LARGER TREES=',F6.2)
      ...
      GO TO 10
C
C  ==========  OPTION NUMBER 120: CRUZFILE ==========================CRUZFILE
C
12000 CONTINUE
      IDT=1
      IF (LNOTBK(1)) IDT=IFIX(ARRAY(1))
      NP=1
      IF(LNOTBK(2)) NP=2
      CALL OPNEW (KODE,IDT,205,NP,ARRAY(2))
      IF (KODE.GT.0) GOTO 10
      IF(NP.EQ.2 .AND. ARRAY(2).EQ.1) THEN
        IF(LKECHO)WRITE(JOSTND,12010) KEYWRD,IDT
12010   FORMAT (/1X,A8,'   DATE/CYCLE=',I5,
     &'; CYCLE 0 CRUISE FILE SUPPRESSED; CYCLE 1 CRUISE FILE DONE')
      ELSEIF (NP.EQ.2 .AND. ARRAY(2).EQ.2) THEN
        IF(LKECHO)WRITE(JOSTND,12015) KEYWRD,IDT
12015   FORMAT (/1X,A8,'   DATE/CYCLE=',I5,
     &'; CYCLE 0 CRUISE FILE DONE; CYCLE 1 CRUISE FILE SUPPRESSED')
      ELSE
        IF(LKECHO)WRITE(JOSTND,12020) KEYWRD,IDT
12020   FORMAT (/1X,A8,'   DATE/CYCLE=',I5)
      ENDIF
      GOTO 10
C
...
12110 FORMAT(/1X,A8,'   DATA BASE CONTROL NUMBER=',A)
      GOTO 10
C
...
12215   FORMAT (/1X,A8,'   DATE/CYCLE=',I5,'; TREES WITH A DMR OF 1-6',
     &          ' WILL BE REMOVED.')
      ELSE
        IF(LKECHO)WRITE(JOSTND,12220) KEYWRD,IDT,PRMS(1)
12220   FORMAT (/1X,A8,'   DATE/CYCLE=',I5,'; TREES WITH A DMR OF',F5.0,
     &          ' WILL BE REMOVED.')
      ENDIF
      IF(LKECHO)WRITE(JOSTND,12225)PRMS(2)*INtoCM,PRMS(3)*INtoCM,PRMS(4)
12225 FORMAT(1X,T13,
     >   'DBH OF REMOVED TREES WILL RANGE FROM ',F5.1,' TO ',
     >   F5.1,' CM.',
     >   ' PROPORTION OF SELECTED TREES REMOVED= ',F6.3)
...
C
     ...
     &             ARRAY,KARD)
      ...
      LMORT=.TRUE.
      CAPDBH=999.*CMtoIN
      IF(LNOTBK(2))CAPDBH=ARRAY(2)*CMtoIN
      CAPPRP=1.0
...
12301   CONTINUE
        ILEN=ISPGRP(-ISPC,52)
        IF(LKECHO)WRITE(JOSTND,12320) KEYWRD,KARD(1)(1:ILEN),ISPC,
     >    CAPDBH*INtoCM,CAPPRP,IDFLG,CAPHT*FTtoM
C----------
C     IF (ISPC=0) ALL SPECIES WILL BE CHANGED IN FOLLOWING CODE:
C----------
      ELSEIF(ISPC .EQ. 0)THEN
        ...
        SIZCAP(I,4)=CAPHT
12310   CONTINUE
        IF(LKECHO)WRITE(JOSTND,12315) KEYWRD,CAPDBH*INtoCM,CAPPRP,
     >    IDFLG,CAPHT*FTtoM
12315   FORMAT(/1X,A8,'   SPECIES= ALL (CODE=  0',
...
C----------
      ELSE
        ...
        IF(LKECHO)WRITE(JOSTND,12320) KEYWRD,KARD(1)(1:ILEN),ISPC,
     >    CAPDBH*INtoCM,CAPPRP,IDFLG,CAPHT*FTtoM
12320   FORMAT(/1X,A8,'   SPECIES= ',A,' (CODE= ',I3,
     >  ');  DBH=',F6.1,';  MORTALITY PROPORTION=',F7.4,
     >  ';  DBH USE CODE=',I2,';  HT=',F6.1)
      ENDIF
      GO TO 10
...
12420 FORMAT (/1X,A8,'   DATE/CYCLE=',I5,'; RESIDUAL=',F8.2) 
      IF(PRMS(2).EQ.0.)THEN
        ILEN=3
        IF(IS.LT.0)ILEN=ISPGRP(-IS,52)
        IF(LKECHO)WRITE(JOSTND,12421)KARD(4)(1:ILEN),IS,
     >    PRMS(4)*INtoCM,
     >    PRMS(5)*INtoCM
12421   FORMAT(1X,T13,
     >   'PROPORTION OF SELECTED TREES REMOVED WILL BE ',
...
12423   FORMAT(1X,T13,'CUTTING WILL BE DISTRIBUTED THROUGHOUT THE ',
     >                'SPECIFIED DIAMETER CLASS.')
      ELSEIF (PRMS(6) .EQ. 1) THEN
        IF(LKECHO)WRITE(JOSTND,12424)
12424   FORMAT(1X,T13,'CUTTING WILL BE FROM BELOW IN THE SPECIFIED ',
     >                'DIAMETER CLASS.')
      ELSE
        IF(LKECHO)WRITE(JOSTND,12425)
12425   FORMAT(1X,T13,'CUTTING WILL BE FROM ABOVE IN THE SPECIFIED ',
     >                'DIAMETER CLASS.')
      ENDIF
      GOTO 10
C
...
C
12700 CONTINUE
      IF (LNOTBK(1)) SDLO=ARRAY(1)*CMtoIN
      IF (LNOTBK(2)) SDHI=ARRAY(2)*CMtoIN
     C
C     SPECIES CODE PROCESSING.
C
...
12708   FORMAT(/1X,A8)
     ...
     >                               IS,'IS'
12709     FORMAT (T13,A,A,' (CODE= ',I3,') ',A,
     >           ' TARGETED FOR SCREENING.')
        ELSE
         IF(LKECHO)WRITE(JOSTND,1411) 'ALL',' SPECIES',IS,'ARE'
        ENDIF
        WRITE(JOSTND,12710) SDLO,SDHI
12710   FORMAT(T13,'ONLY RECORDS WITH DIAMETERS GE',F6.1,
     &     ' CM OR LT',F6.1,' CM WILL BE READ FROM THE DATA.',/,
     &     T13,'ALL OTHER INPUT RECORDS WILL BE SCREENED OUT.')
      ENDIF
      GOTO 10      
C
C  ==========  OPTION NUMBER 128: SETPTTHIN ========================SETPTTHIN
C
12800 CONTINUE
      ICFLAG=248
      IDT=1
...
12811   FORMAT(T13,'POINT NUMBER IS MISSING.')
        GO TO 10
      ENDIF
      IF(PRMS(2).LE.0. .OR. PRMS(2).GT.5)THEN
        CALL KEYDMP (JOSTND,IRECNT,KEYWRD,ARRAY,KARD)
        CALL ERRGRO (.TRUE.,4)
        IF(LKECHO)WRITE(JOSTND,12814)
12814   FORMAT(T13,'POINT THINNING RESIDUAL ATTRIBUTE IS MISSING ',
     &  'OR INVALID.')
        GO TO 10
      ENDIF
C
...
12920 FORMAT (/1X,A8,'   DATE/CYCLE=',I5,'; RESIDUAL=',F8.2)
      IF(PRMS(2).EQ.0.)THEN
        ILEN=3
        IF(IS.LT.0)ILEN=ISPGRP(-IS,52)
        IF(LKECHO)WRITE(JOSTND,12921)KARD(4)(1:ILEN),IS,
     >  PRMS(4)*INtoCM, PRMS(5)*INtoCM
12921   FORMAT(1X,T13,
     >   'PROPORTION OF SELECTED TREES REMOVED WILL BE ',
     >   'COMPUTED BY THE MODEL'/,
     >   T13,'SPECIES=',A,' (CODE=',I3,');',
     ...
     >   F5.1,' CM.')
      ELSE
        ILEN=3
        IF(IS.LT.0)ILEN=ISPGRP(-IS,52)
        IF(LKECHO)WRITE(JOSTND,12922)PRMS(2),KARD(4)(1:ILEN),IS,
     >  PRMS(4)*INtoCM, PRMS(5)*INtoCM
12922   FORMAT(1X,T13,
     >   'PROPORTION OF SELECTED TREES REMOVED= ',F6.3,';',/,
     >   T13,'SPECIES=',A,' (CODE=',I3,');',
...
12923   FORMAT(1X,T13,'CUTTING WILL BE DISTRIBUTED THROUGHOUT THE ',
     >                'SPECIFIED DIAMETER CLASS,')
      ELSEIF (PRMS(6) .EQ. 1) THEN
        IF(LKECHO)WRITE(JOSTND,12924)
12924   FORMAT(1X,T13,'CUTTING WILL BE FROM BELOW IN THE SPECIFIED ',
     >                'DIAMETER CLASS,')
      ELSE
        IF(LKECHO)WRITE(JOSTND,12925)
12925   FORMAT(1X,T13,'CUTTING WILL BE FROM ABOVE IN THE SPECIFIED ',
     >                'DIAMETER CLASS,')
      ENDIF
      IF(LKECHO)WRITE(JOSTND,12926)
12926 FORMAT(1X,T13,'ON THE POINT SPECIFIED WITH THE SETPTTHIN ',
...
C
      IF((VVER(:2).EQ.'SN').OR.(VVER(:2).EQ.'SE'))THEN
        IREGN = KODFOR/10000
...
C
        ...
        ENDIF
        GOTO 10
13003   FORMAT(/1X,A8,'   INVALID VOLUME EQUATION NUMBER: THE DEFAULT ',
     &    'CUBIC FT EQUATION NUMBER WILL BE USED FOR SPECIES GROUP=',A,
     &    ' (CODE=',I3,');')
13004   FORMAT(/1X,A8,'   INVALID VOLUME EQUATION NUMBER: THE DEFAULT ',
     &    'BOARD FT EQUATION NUMBER WILL BE USED FOR SPECIES GROUP=',A,
     &    ' (CODE=',I3,');')
13008   FORMAT(/1X,A8,'   NATIONAL CRUISE SYSTEM CUBIC FT VOLUME EQUA',
     &  'TION NUMBERS CHANGED FOR SPECIES GROUP=',A,' (CODE=',I3,');')
13009   FORMAT(/1X,A8,'   NATIONAL CRUISE SYSTEM BOARD FT VOLUME EQUA',
     &  'TION NUMBERS CHANGED FOR SPECIES GROUP=',A,' (CODE=',I3,');')
C----------
...
C
        ...
        ENDIF
        GOTO 10
13006   FORMAT(/1X,A8,'   INVALID VOLUME EQUATION NUMBER: THE',
     &    ' DEFAULT CUBIC FT EQUATION NUMBERS WILL BE USED FOR ALL ',
     &    'SPECIES:')
C       IF(LKECHO)WRITE(JOSTND,13021)KEYWRD
13020   FORMAT(/1X,A8,'   NATIONAL CRUISE SYSTEM CUBIC FT ',
     &    'VOLUME EQUATION NUMBERS CHANGED FOR ALL SPECIES:')
13016   FORMAT(/1X,A8,'   INVALID VOLUME EQUATION NUMBER: THE',
     &    ' DEFAULT BOARD FT EQUATION NUMBERS WILL BE USED FOR ALL ',
     &    'SPECIES:')
C       IF(LKECHO)WRITE(JOSTND,13021)KEYWRD
13021   FORMAT(/1X,A8,'   NATIONAL CRUISE SYSTEM BOARD FT VOLUME ',
...
C
     ...
     &      WRITE(JOSTND,13030)KEYWRD,KARD(1)(1:3),IS
          ENDIF
        ENDIF
C13005       FORMAT(/1X,A8,'   INVALID CUBIC FT VOLUME EQUATION ',
C     &      'NUMBER: THE DEFAULT EQUATION NUMBER WILL BE USED ',
C     &      'FOR SPECIES=',A,' (CODE=',I3,');')
C13007       FORMAT(/1X,A8,'   INVALID BOARD FT VOLUME EQUATION ',
C     &      'NUMBER: THE DEFAULT EQUATION NUMBER WILL BE USED ',
C     &      'FOR SPECIES=',A,' (CODE=',I3,');')
13030     FORMAT(/1X,A8,'   NATIONAL CRUISE SYSTEM VOLUME EQUATION ',
     &    'NUMBERS CHANGED FOR SPECIES=',A,' (CODE=',I3,');')
      ENDIF
      IF(LNOTBK(2).AND.LNOTBK(3))THEN
        IF(LKECHO)WRITE(JOSTND,13040)KARD(2),KARD(3)
13040   FORMAT(T10,'   CUBIC FOOT = ',A10,'   BOARD FOOT = ',A10)
      ELSEIF(LNOTBK(2))THEN
        IF(LKECHO)WRITE(JOSTND,13041)KARD(2)
13041   FORMAT(T10,'   CUBIC FOOT = ',A10)
      ELSEIF(LNOTBK(3))THEN
        IF(LKECHO)WRITE(JOSTND,13042)KARD(3)
13042   FORMAT(T10,'   BOARD FOOT = ',A10)
      ELSE
        WRITE(JOSTND,13043)
13043   FORMAT(T10,' EQUATION NUMBERS NOT SPECIFIED.',
     &  '  REQUEST CANCELLED.')
      ENDIF
      GO TO 10
C
...
13100 CONTINUE
      ...
      IF(ITHNPI.LT.0 .OR. ITHNPI.GT.2) THEN
        ...
        CALL ERRGRO (.TRUE.,4)
        WRITE(JOSTND,13105)
13105   FORMAT(T13,'INVALID POINT INDICATOR. POINT INDICATOR BEING ',
...
13110   FORMAT(/1X,A8,'   POINT NUMBERS ARE AS INPUT IN THE TREE DATA.')
      ELSE
        IF(LKECHO)WRITE(JOSTND,13111)KEYWRD
13111   FORMAT(/1X,A8,'   POINT NUMBERS ARE THE FVS SEQUENTIAL POINT ',
     &  'NUMBERS.')
      ENDIF
      GO TO 10
C
...
13210 FORMAT(/1X,A8,'   THERE WERE',I8,' KEYWORD RECORDS PROCESSED.',
...
13310   FORMAT(/1X,A8,'   KEYWORDS WILL **NOT** BE ECHOED TO THE MAIN',
     &                ' OUTPUT FILE.')
      GO TO 10
C
...
13410      FORMAT(/1X,A8,'   REQUESTED YEAR FOR A CYCLE =',I4)
        ENDIF
        GO TO 10
...
13510 FORMAT (/1X,A8,'   DATE/CYCLE=',I5,'; DATA SET REFERENCE ',
     >        'NUMBER =',F4.0,'; HEADING SUPPRESSION CODE =',F3.0/
     >        T13,'(0=WITH HEADING, OTHER VALUES=SUPPRESS HEADING).')
      GOTO 10
C
...
13700 CONTINUE
      ...
      ENDIF
      IF(LNOTBK(4))THEN
        IF(ARRAY(4) .GE. 0.) DLOMSB=ARRAY(4)*CMtoIN
      ENDIF
      IF(LNOTBK(5))THEN
        IF(ARRAY(5) .GE. 0.) DHIMSB=ARRAY(5)*CMtoIN
      ENDIF
      ...
      IF(LNOTBK(6))THEN
        ...
        ENDIF
      ENDIF
      IF(LKECHO)WRITE(JOSTND,13710) KEYWRD,QMDMSB*INtoCM,SLPMSB,
     >  EFFMSB,DLOMSB*INtoCM,DHIMSB*INtoCM,MFLMSB
13710 FORMAT (/1X,A8,'   MATURE STAND BOUNDARY: QMD =',F9.4,
     ...
     >';  MORTALITY FLAG = ',I1)
      ...
      ELSE
        WRITE(JOSTND,13713)
      ENDIF
13711 FORMAT(12X,'(MORTALITY WILL BE FROM ABOVE WITHIN THE SPECIFIED',
     &' DBH RANGE.)')
13712 FORMAT(12X,'(MORTALITY WILL BE FROM BELOW WITHIN THE SPECIFIED',
     &' DBH RANGE.)')
13713 FORMAT(12X,'(MORTALITY WILL BE SPREAD THROUGHOUT THE SPECIFIED',
     &' DBH RANGE.)')
      GOTO 10
C
C RESTORE DEFAULTS
...
13830 FORMAT (/1X,A8,'   DATE/CYCLE=',I5)
C
      IF(ARRAY(2).NE.0 .AND. LKECHO)THEN
        WRITE(JOSTND,13832)IHAB
13832   FORMAT (T13,'HABITAT TYPE=',I5)
      ENDIF
C
      IF(ARRAY(3).NE.0 .AND. LKECHO)THEN
        WRITE(JOSTND,13836)ARRAY(3)
13836   FORMAT (T13,'BASAL AREA MAXIMUM=',F5.0)
      ENDIF
C
      IF(ARRAY(5).NE.0 .AND. LKECHO)THEN
        WRITE(JOSTND,13833)KARD(4)(1:ILEN),IS,ARRAY(5)
13833   FORMAT (T13,'SPECIES=',A,' (CODE=',I3,'); SITE INDEX=',F10.1)
        IF(ARRAY(6).EQ.0)THEN
          WRITE(JOSTND,13834)
13834     FORMAT(T13,'SITE INDEX VALUE IS ENTERED DIRECTLY')
        ELSE
          WRITE(JOSTND,13835)
13835     FORMAT(T13,'SITE INDEX VALUE ENTERED AS A PERCENT CHANGE')
        ENDIF
      ENDIF
C
      IF(ARRAY(7).NE.0 .AND. LKECHO)THEN
        WRITE(JOSTND,13837)ARRAY(7)
13837   FORMAT (T13,'STAND DENSITY INDEX=',F5.0)
      ENDIF
C
...
14000 CONTINUE
     ...
     &       'EC','EM','NC','NE','SN','SO','TT','UT','WS','ON')
        DBHSTAGE=0.
        IF(LNOTBK(1))DBHSTAGE=ARRAY(1)*CMtoIN
        DBHZEIDE=1.
        IF(LNOTBK(2))DBHZEIDE=ARRAY(2)*CMtoIN
