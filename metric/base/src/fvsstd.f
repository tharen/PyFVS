...
C----------
      ...
      KSDI=(TEMSDI/ACRtoHA)/GROSPC + 0.5
C----------
...
C
     ...
     >  ISPECL(I),ITRE(I),P/ACRtoHA,DP/ACRtoHA,DBH(I)*INtoCM,
     >  DGI*INtoCM,HT(I)*FTtoM,HTG(I)*FTtoM,ICR(I),CW*FTtoM,
     >  IDMR,PCT(I),IPTBAL,CFV(I)*FT3toM3,WK1(I)*FT3toM3,
     >  BFV(I)*FT3toM3,ICDF,IBDF,
     >  NINT(FLOAT(ITRUNC(I)+5)*.01*FTtoM),PDBHI*INtoCM,PHTI*FTtoM,
     >  PTOCFV(I)*FT3toM3,PMRCFV(I)*FT3toM3,PMRBFV(I)*FT3toM3,
     >  NCFDEF(I),NBFDEF(I),YRDLOS(I),
     >  NINT(FLOAT(NORMHT(I)+5)*.01*FTtoM),ABIRTH(I),IPVEC(ITRE(I))
