      ...
      implicit none
C----------
C  **PRTRLS--LS/M     DATE OF LAST REVISION: 06/08/10
C----------
C
...
C
      IF (LFORMT) THEN
        IF(NPRMS.LT.5 .OR. TEM(5).LE.0.)THEN
   ...
   21     FORMAT(1X,A8,1X,I4,1X,A2,I3,2(1X,I2),1X,I3,1X,F8.3,1X,F8.3,1X,
     ...
     >       1X,I3)
        ELSE
          WRITE(KOLIST,20) TID,I,ISP(I),ITRE(I),
     >      P/ACRtoHA,
     >      DBH(I)*INtoCM,
     >      DGI*INtoCM,
     >      HT(I)*FTtoM,
     >      HTG(I)*FTtoM,
     >      IICR,ICR(I),PCT(I),IMC(I),
     >      CFV(I)*FT3toM3,
     >      WK1(I)*FT3toM3,
     >      BFV(I)*FT3toM3,
     >      FLOAT(ITRUNC(I))*.01*FTtoM,
     >      ICYC,IESTAT(I),IDMR
   20     FORMAT(1X,A8,1X,2I4,I5,F9.3,F8.2,3F7.2,I3,I4,F9.3,I4,F9.2,
     ...
     >       NINT(FLOAT(ITRUNC(I)+5)*.01*FTtoM)
        ELSE
          WRITE(KOLIST) TID,I,ISP(I),ITRE(I),
     >      P/ACRtoHA,
     >      DBH(I)*INtoCM,
     >      DGI*INtoCM,
     >      HT(I)*FTtoM,
