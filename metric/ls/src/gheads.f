      SUBROUTINE GHEADS(NPLT,MGMID,JOSTND,JOTREE,ITITLE)
      IMPLICIT NONE
C----------
C  **GHEADS--LS/M DATE OF LAST REVISION:   06/02/08
C----------
C
C     PRINTS HEADINGS. IF I/O UNIT(S) IS/ARE ZERO THE PRINTING
C     OF THE CORRESPONDING HEADING IS SKIPPED.
C
      CHARACTER*26 NPLT
      CHARACTER*72 ITITLE
      CHARACTER*4 MGMID
      INTEGER JOTREE,JOSTND,ISTLNB
C
      IF(JOSTND.EQ.0) GO TO 10
C
C     WRITE HEADING FOR STAND COMPOSITION OUTPUT.
C
      CALL GROHED(JOSTND)
      WRITE (JOSTND,5) NPLT,MGMID,ITITLE(1:ISTLNB(ITITLE))
    5 FORMAT(/' STAND ID: ',A26,4X,'MGMT ID: ',A4,4X,A/)
      CALL PPLABS (JOSTND)
      WRITE (JOSTND,50)
   50 FORMAT(/T42,
     >'STAND COMPOSITION (BASED ON STOCKABLE AREA)'/
     >1X,122('-')/T30,'PERCENTILE POINTS IN THE'/
     >T23,'DISTRIBUTION OF STAND ATTRIBUTES BY DBH       TOTAL/HA'/
     >T10,'STAND',7X,41('-'),6X,'OF STAND',9X,
     >'DISTRIBUTION OF STAND ATTRIBUTES BY'/1X,'YEAR  ATTRIBUTES',
     >6X,'10     30     50     70     90    100',7X,'ATTRIBUTES',7X,
     >'SPECIES AND 3 USER-DEFINED SUBCLASSES'/1X,4('-'),2X,11('-'),3X,
     >6(6('-'),1X),2X,14('-'),2X,42('-')/T35,'(DBH IN CMS)')
C
   10 CONTINUE
      IF(JOTREE .EQ. 0) RETURN
C
C     WRITE LABELS FOR EXAMPLE TREE OUTPUT.
C
      CALL GROHED(JOTREE)
      WRITE (JOTREE,5) NPLT,MGMID,ITITLE(1:ISTLNB(ITITLE))
      CALL PPLABS (JOTREE)
      WRITE (JOTREE,80)
   80 FORMAT(
     >/1X,126('-')
     >/T23,'ATTRIBUTES OF SELECTED SAMPLE TREES',16X,
     >' ADDITIONAL STAND ATTRIBUTES (BASED ON STOCKABLE AREA)'/
     >7X,65('-'),2X,53('-')/7X,'INITIAL',
     >27X,'LIVE   PAST DBH   BASAL  TREES',10X,'QUADRATIC   TREES',
     >4X,'BASAL  TOP HEIGHT CROWN'/6X,'TREES/HA',11X,'DBH     HEIGHT',
     >2X,'CROWN   GROWTH    AREA    PER    STAND  MEAN DBH     PER',
     >5X,' AREA    LARGEST  COMP '/' YEAR   %TILE  SPECIES  (CMS) ',
     >'  (METRES) RATIO   (CMS)     %TILE   HA      AGE    (CMS)',4X,
     >'   HA     (SQM/HA) 100/HA(M) FACTOR'/1X,'----',2X,2(7('-'),1X),
     >2(8('-'),1X),6('-'),1X,9('-'),1X,7('-'),1X,6('-'),2X,5('-'),2X,
     >9('-'),3X,6('-'),1X,2(9('-'),1X),7('-'))
C
      RETURN
C
C  CALLED FROM **SITSET** ROUTINES TO WRITE TABLE HEADERS FR THE
C  1) FIAHEAD   - THE SPECIES FIA NUMBER / ALPHA CODE TRANSLATION TABLE
C  2) VOLEQHEAD - VOLUME EQUATION NUMBERS
C
      ENTRY FIAHEAD(JOSTND)
      WRITE(JOSTND,210)
 210  FORMAT(/,' ',T13,'ALPHA SPECIES - FIA CODE CROSS REFERENCE:')
      RETURN
C
      ENTRY VOLEQHEAD(JOSTND)
      WRITE(JOSTND,220)
  220 FORMAT(/,T36,'NATIONAL VOLUME ESTIMATOR LIBRARY EQUATION NUMBERS',
     &/,1X,4('SPECIES CUBIC FOOT BOARD FOOT ')/
C     & 1X,4('          EQ. NO.    EQ. NO.  ')/
     &  1X,4('------- ---------- ---------- '))
C
      RETURN
      END
