      ...
      implicit none
C----------
C    ESIN--ESTB/M  DATE OF LAST REVISION:   01/13/09
C----------
C
...
C
 ...
 1080 CONTINUE
      IF (NUMBER.NE.1) THEN
         ...
         ELSE
C
C           IF THE NEWEST ESTAB KEYWORD CONTAINS A DATE OF DIST, THEN
...
C
      CALL SPDECD (2,IS,NSP(1,1),MAXSP,JOSTND,IRECNT,KEYWRD,
     &             ARRAY,KARD)
 ...
 1230 FORMAT(12X,'NATURAL IMPLIES: STOCKADJ = 0.0, NOAUTALY, ',
     >           'AND NOINGROW.')
      GOTO 10
 1300 CONTINUE
C
...
C
 ...
 2310    FORMAT(/1X,A8,'   DATE/CYCLE=',I5,'; MULTIPLIER=',F6.2)
         IF((ARRAY(2).LE.0.).AND.LKECHO)WRITE(JOSTND,2320)
 2320    FORMAT (T13,'PREDICTION OF NATURAL REGENERATION IS CANCELLED.')
      ENDIF
      GOTO 10
 2400 CONTINUE
C
...
C
     ...
     &             ARRAY,KARD)
      ...
      ILEN=3
      IF(IS.LT.0)ILEN=ISPGRP(-IS,52)
      IF(LKECHO)WRITE(JOSTND,2530) KEYWRD,IDT,KARD(2)(1:ILEN),
     &  IS,ARRAY(3)*FTtoM
...
C
 ...
 3510 FORMAT(/1X,A8,'   THRESHOLD VALUES FOR AUTOMATIC TALLIES ',
     &  'FOLLOWING THINNINGS:',/,T13,'LOWER PERCENT=',F6.1,
     &  '   UPPER PERCENT=',F6.1)
      GOTO 10
 3600 CONTINUE
C
...
C*********************************************************************
 ...
 9010 FORMAT (T13,'STOCKADJ IS SET TO ZERO, AND NO SPROUTING ',
     >       'WILL BE SIMULATED.')
      RETURN
C
