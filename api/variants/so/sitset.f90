...
C  FOR REGION 6 FORESTS, AND WARM SPRINGS, SET SDI DEFAULTS HERE ALSO.
C----------
...
C  WARM SPRINGS RESERVATION (IFOR=10) IS ALSO CONSIDERED A R6 FOREST
C  KLAMATH, LASSEN, MODOC, AND PLUMAS (IFOR=4,5,6,7) ARE R5 FORESTS.
C  SHASTA (IFOR=9) GETS MAPPED TO IFOR=4 IN **FORKOD**
C  INDUSTRY (IFOR=8) USE R5 DEFAULTS.
C----------
      DO 50 ISPC=1,MAXSP
...
!----------
      ...
      VAR='SO'
C
C  USE R5 EQNS FOR INDUSTRY LANDS; R6 EQNS FOR WARM SPRINGS
C  HERE AND IN **INITRE** VOLEQNUM PROCESS
C    INDUSTRY (KODFOR=701) USES KLAMATH (KODFOR=505)
C    WARM SPRINGS (KODFOR=799) USES DESCHUTES (KODFOR=601)
C
      IF(IREGN.EQ.7)THEN
        SELECT CASE (KODFOR)
        CASE (701)
