      ...
      implicit none
C----------
C  **MORTS--BM   DATE OF LAST REVISION:  09/09/13
C----------
...
C----------
      INTEGER MYACTS(2),IDMFLG,KNT,NTODO,I,NP,IACTK,IDATE,ISPCC
      INTEGER IPASS,ISPC,I1,I2,I3,IS,IPATH,KNT2
      INTEGER ITODO,IP,IGRP,IULIM,IG,IX,J,KPOINT,KBIG
      REAL D,TNEW,BANEW,QMDNEW,ADJFAC,XMORE,TEMP,CREDIT,TN10TMP
      REAL PMSC(MAXSP),PMD(MAXSP),PRM(6),TMD0,T85D0,T55D0,TMD10
      REAL T85D10,T55D10,TN10,TEM,D55M,T55M,D85M,T85M,SLP,CEPT
      REAL TPRIME,DIFF,RN,XMORT,D1,D2,B0,B1,WKI,RI,RIP,X,PRES
      REAL VLOS,SUMTRE,TN,SD2SQN,DQ10N,TREEIT,DQ0,SUMKIL,T
      REAL SD2SQ,P,BARK,BRATIO,G,CIOBDS,DQ10,DELTBA,CONST
...
C NOTE: FOR SPECIES FROM THE WC VARIANT (WHICH USES A DIFFERENT TYPE OF
C MORTALITY MODEL), SO USES SURROGATE COEFFICIENTS FROM OTHER SO
C SPECIES: PY AND CW USE PP; OH USES LP: THE SAME IS DONE HERE.
C ALSO, YC USES SO IC
C----------
      DATA PMSC/6.5112, 6.5112, 7.2985, 5.1677, 9.6943,
     &          5.1677, 5.9617, 9.6943, 5.1677, 5.5877,
     &          6.5112, 6.5112, 5.5877, 5.5877, 5.1677,
     &          5.5877, 5.5877, 5.9617/
      DATA PMD/-.0052485, -.0052485, -.0129121, -.0077681, -.0127328,
     &         -.0077681, -.0340128, -.0127328, -.0077681,  -.005348,
     &         -.0052485, -.0052485,  -.005348,  -.005348, -.0077681,
     &         -0.005348,  -.005348, -.0340128/
C----------
C  INITIALIZE.
C----------
...
C----------
     ...
     &TMD10,PMSDIU,PMSDIL,T85D10,T55D10
      IF(DEBUG)WRITE(JOSTND,9040)ICYC,SDIMAX,BAMAX
 9040 FORMAT('IN MORTS ICYC,MAX SDI, BAMAX  =',I5,2F10.1)
C----------
...
C----------
      ...
      IF(TN10 .GE. 0.1)THEN
        CALL BMTMRT(SUMTRE,DEBUG,SUMKIL)
      ENDIF
C----------
